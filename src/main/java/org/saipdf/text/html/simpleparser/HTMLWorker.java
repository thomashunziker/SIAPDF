...
package org.saipdf.text.html.simpleparser;
...
import org.saipdf.text.ElementTags;
import org.saipdf.text.ExceptionConverter;
import org.saipdf.text.FontProvider;
...
import org.saipdf.text.TextElementArray;
import org.saipdf.text.html.HtmlTags;
import org.saipdf.text.html.Markup;
import org.saipdf.text.pdf.PdfPTable;
import org.saipdf.text.pdf.draw.LineSeparator;
	...
	public void startElement(String tag, HashMap h) {
		...
		try {
			...
			if (tag.equals(HtmlTags.UNORDEREDLIST)) {
				...
				skipText = true;
				cprops.addToChain(tag, h);
				org.saipdf.text.List list = new org.saipdf.text.List(false);
				try {
			...
			if (tag.equals(HtmlTags.ORDEREDLIST)) {
				...
				skipText = true;
				cprops.addToChain(tag, h);
				org.saipdf.text.List list = new org.saipdf.text.List(true);
				try {
					list.setIndentationLeft(new Float(cprops.getProperty("indent")).floatValue());
				} catch (Exception e) {
					list.setAutoindent(true);
		...
		try {
			...
			if (tag.equals(HtmlTags.UNORDEREDLIST) || tag.equals(HtmlTags.ORDEREDLIST)) {
				...
				if (stack.empty())
					return;
				Object obj = stack.pop();
				if (!(obj instanceof org.saipdf.text.List)) {
					stack.push(obj);
					return;
				}
				if (stack.empty())
			...
			if (tag.equals(HtmlTags.LISTITEM)) {
				...
				if (!(list instanceof org.saipdf.text.List)) {
					stack.push(list);
					return;
				}
				ListItem item = (ListItem) obj;
				((org.saipdf.text.List) list).add(item);
