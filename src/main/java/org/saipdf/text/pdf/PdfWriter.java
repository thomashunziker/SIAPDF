...
import org.saipdf.text.DocListener;
import org.saipdf.text.DocWriter;
import org.saipdf.text.Document;
import org.saipdf.text.DocumentException;
import org.saipdf.text.ExceptionConverter;
import org.saipdf.text.Image;
import org.saipdf.text.ImgJBIG2;
import org.saipdf.text.ImgWMF;
import org.saipdf.text.Rectangle;
import org.saipdf.text.Table;
import org.saipdf.text.error_messages.MessageLocalization;
import org.saipdf.text.pdf.collection.PdfCollection;
import org.saipdf.text.pdf.events.PdfPageEventForwarder;
import org.saipdf.text.pdf.interfaces.PdfAnnotations;
import org.saipdf.text.pdf.interfaces.PdfDocumentActions;
import org.saipdf.text.pdf.interfaces.PdfEncryptionSettings;
import org.saipdf.text.pdf.interfaces.PdfPageActions;
import org.saipdf.text.pdf.interfaces.PdfRunDirection;
import org.saipdf.text.pdf.interfaces.PdfVersion;
import org.saipdf.text.pdf.interfaces.PdfViewerPreferences;
import org.saipdf.text.pdf.interfaces.PdfXConformance;
import org.saipdf.text.pdf.internal.PdfVersionImp;
import org.saipdf.text.pdf.internal.PdfXConformanceImp;
import org.saipdf.text.xml.xmp.XmpWriter;


/**
 * A <CODE>DocWriter</CODE> class for PDF.
 * <P>
 * When this <CODE>PdfWriter</CODE> is added to a certain
 * <CODE>PdfDocument</CODE>, the PDF representation of every Element added to
 * this Document will be written to the outputstream.
 * </P>
 */

public class PdfWriter extends DocWriter implements PdfViewerPreferences, PdfEncryptionSettings, PdfVersion, PdfDocumentActions, PdfPageActions,
		PdfXConformance, PdfRunDirection, PdfAnnotations {
	 ...
	 * <P>
	 * This class covers the third section of Chapter 5 in the 'Portable
	 * Document Format Reference Manual version 1.3' (page 55-60). It contains
	 * the body of a PDF document (section 5.14) and it can also generate a
	 * Cross-reference Table (section 5.15).
	 * 
	 * @see PdfWriter
	 * @see PdfObject
	...
	public static class PdfBody {
		...
		/**
		 * <CODE>PdfCrossReference</CODE> is an entry in the PDF Cross-Reference
		 * table.
		 */
		 ...
		 * <P>
		 * This methods creates a <CODE>PdfIndirectObject</CODE> with a certain
		 * number, containing the given <CODE>PdfObject</CODE>. It also adds a
		 * <CODE>PdfCrossReference</CODE> for this object to an
		 * <CODE>ArrayList</CODE> that will be used to build the Cross-reference
		 * Table.
		 * 
		 * @param object
		 *            a <CODE>PdfObject</CODE>
		...
		/**
		 * Gets a PdfIndirectReference for an object that will be created in the
		 * future.
		 * 
		 ...
		 * <P>
		 * This methods creates a <CODE>PdfIndirectObject</CODE> with the number
		 * given by <CODE>ref</CODE>, containing the given
		 * <CODE>PdfObject</CODE>. It also adds a <CODE>PdfCrossReference</CODE>
		 * for this object to an <CODE>ArrayList</CODE> that will be used to
		 * build the Cross-reference Table.
		 * 
		 * @param object
		 *            a <CODE>PdfObject</CODE>
		 * @param ref
		 *            a <CODE>PdfIndirectReference</CODE>
		 * @return a <CODE>PdfIndirectObject</CODE>
		 * @throws IOException
		 */
		 ...
		 * CrossReferenceTable of this <CODE>Body</CODE>.
		 * 
		...
		void writeCrossReferenceTable(OutputStream os, PdfIndirectReference root, PdfIndirectReference info, PdfIndirectReference encryption, PdfObject fileID,
	 ...
	 * Manual version 1.3' section 5.16 (page 59-60).
	 */
	...
	static class PdfTrailer extends PdfDictionary {
		 ...
		 *            <CODE>PdfCrossReferenceTable</CODE>
		 * @param offset
		 *            offset of the <CODE>PdfCrossReferenceTable</CODE>
		 * @param root
		 *            an indirect reference to the root of the PDF document
		 * @param info
		 *            an indirect reference to the info object of the PDF
		 *            document
		 * @param encryption
		 * @param fileID
		 * @param prevxref
	 ...
	 *            The <CODE>OutputStream</CODE> the writer has to write to.
	 * @param listener
	 *            A <CODE>DocListener</CODE> to pass to the PdfDocument.
	 * @throws DocumentException
	 *             on error
	 */
	...
	/**
	 * Use this method to get the info dictionary if you want to change it
	 * directly (add keys and values to the info dictionary).
	 * 
	 ...
	 * 
	 * @param ensureNewLine
	 *            Tells whether a new line shall be enforced. This may cause
	 *            side effects for elements that do not terminate the lines
	 *            they've started because those lines will get terminated.
	...
	/**
	 * Sets the initial leading for the PDF document. This has to be done before
	...
	/*
	 * You should see Direct Content as a canvas on which you can draw graphics
	 * and text. One canvas goes on top of the page (getDirectContent), the
	 * other goes underneath (getDirectContentUnder). You can always the same
	 * object throughout your document, even if you have moved to a new page.
	 * Whatever you add on the canvas will be displayed on top or under the
	 * current page.
	 */
	...
	/**
	 * Use this method to get the direct content for this document. There is
	 * only one direct content, multiple calls to this method will allways
	...
	/**
	 * Use this method to get the direct content under for this document. There
	 * is only one direct content, multiple calls to this method will always
	 * retrieve the same object.
	 * 
	 * @return the direct content
	 */
	...
	/**
	 * Resets all the direct contents to empty. This happens when a new page is
	 * started.
	 */
	void resetContent() {
		directContent.reset();
	...
	/*
	 * A PDF file has 4 parts: a header, a body, a cross-reference table, and a
	 * trailer. The body contains all the PDF objects that make up the PDF
	 * document. Each element gets a reference (a set of numbers) and the byte
	 * position of every object is stored in the cross-reference table. Use
	 * these methods only if you know what you're doing.
	 */
	...
	/**
	 * Use this method to add a PDF object to the PDF body. Use this method only
	 ...
	 * will be created in the future. Use this method only if you know what
	 * you're doing!
	 * 
	...
	/*
	 * The Catalog is also called the root object of the document. Whereas the
	 * Cross-Reference maps the objects number with the byte offset so that the
	 * viewer can find the objects, the Catalog tells the viewer the numbers of
	 * the objects needed to render the document.
	 */
	...
	/*
	 * The page root keeps the complete page tree of the document. There's an
	 * entry in the Catalog that refers to the root of the page tree, the page
	 * tree contains the references to pages and other page trees.
	 */
	...
	/**
	 * Use this method to make sure the page tree has a linear structure (every
	 * leave is attached directly to the root). Use this method to allow page
	 * reordering with method reorderPages.
	 */
	...
	/**
	 * Use this method to reorder the pages in the document. A <CODE>null</CODE>
	 * argument value only returns the number of pages to process. It is
	 * advisable to issue a <CODE>Document.newPage()</CODE> before using this
	 * method.
	 * 
	 * @return the total number of pages
	 * @param order
	 *            an array with the new page sequence. It must have the same
	...
	/**
	 * Use this method to get a reference to a page existing or not. If the page
	 * does not exist yet the reference will be created in advance. If on
	 * closing the document, a page number greater than the total number of
	 * pages was requested, an exception is thrown.
	 * 
	 * @param page
	...
	/**
	 * Gets the pagenumber of this document. This number can be different from
	 * the real pagenumber, if you have (re)set the page number previously.
	 * 
	 ...
	 * 
	 * @param tabs
	 *            Can be PdfName.R, PdfName.C or PdfName.S. Since the Adobe
	 *            Extensions Level 3, it can also be PdfName.A or PdfName.W
	 * @since 2.1.5
	 */
	 ...
	 * <P>
	 * The document has to be open before you can begin to add content to the
	 * body of the document.
	 * 
	 * @return a <CODE>PdfIndirectReference</CODE>
	 * @param page
	 *            the <CODE>PdfPage</CODE> to add
	 * @param contents
	 *            the <CODE>PdfContents</CODE> of the page
	 * @throws PdfException
	 *             on error
	 */
	...
	/*
	 * Page events are specific for iText, not for PDF. Upon specific events
	 * (for instance when a page starts or ends), the corresponding method in
	 * the page event implementation that is added to the writer is invoked.
	 */
	 ...
	 * if none is set.
	 * 
	 * @return the <CODE>PdfPageEvent</CODE> for this document or
	 ...
	 * <P>
	 * When this method is called, the PDF-document header is written to the
	 * outputstream.
	 * 
	 * @see org.saipdf.text.DocWriter#open()
	 */
	public void open() {
		super.open();
		try {
	 ...
	 * <P>
	 * The pages-tree is built and written to the outputstream. A Catalog is
	 * constructed, as well as an Info-object, the reference table is composed
	 * and everything is written to the outputstream embedded in a Trailer.
	 * 
	 * @see org.saipdf.text.DocWriter#close()
	 */
	public void close() {
		if (open) {
			if ((currentPageNumber - 1) != pageReferences.size())
				throw new RuntimeException("The page " + pageReferences.size() + " was requested but the document has only " + (currentPageNumber - 1)
						+ " pages.");
			pdf.close();
			try {
				...
				if (fullCompression) {
					...
					PdfTrailer trailer = new PdfTrailer(body.size(), body.offset(), indirectCatalog.getIndirectReference(), infoObj.getIndirectReference(),
	...
	/** @see org.saipdf.text.pdf.interfaces.PdfVersion#setPdfVersion(char) */
	public void setPdfVersion(char version) {
		pdf_version.setPdfVersion(version);
	}

	/** @see org.saipdf.text.pdf.interfaces.PdfVersion#setAtLeastPdfVersion(char) */
	public void setAtLeastPdfVersion(char version) {
		pdf_version.setAtLeastPdfVersion(version);
	}

	/** @see org.saipdf.text.pdf.interfaces.PdfVersion#setPdfVersion(org.saipdf.text.pdf.PdfName) */
	public void setPdfVersion(PdfName version) {
		pdf_version.setPdfVersion(version);
	}

	/**
	 * @see org.saipdf.text.pdf.interfaces.PdfVersion#addDeveloperExtension(org.saipdf.text.pdf.PdfDeveloperExtension)
	 * @since 2.1.6
	 */
	...
	/** @see org.saipdf.text.pdf.interfaces.PdfViewerPreferences#setViewerPreferences(int) */
	public void setViewerPreferences(int preferences) {
		pdf.setViewerPreferences(preferences);
	}

	/**
	 * @see org.saipdf.text.pdf.interfaces.PdfViewerPreferences#addViewerPreference(org.saipdf.text.pdf.PdfName,
	 *      org.saipdf.text.pdf.PdfObject)
	 */
	...
	/**
	 * Adds named destinations in bulk. Valid keys and values of the map can be
	 * found in the map that is created by SimpleNamedDestination.
	 * 
	 * @param map
	 *            a map with strings as keys for the names, and structured
	 *            strings as values for the destinations
	 * @param page_offset
	 *            number of pages that has to be added to the page numbers in
	 *            the destinations (useful if you use this method in combination
	 *            with PdfCopy).
	 * @since iText 5.0
	 */
	...
	/**
	 * Use this method to add a JavaScript action at the document level. When
	 * the document opens, all this JavaScript runs.
	 * 
	 * @param js
	 ...
	 *            the JavaScript code
	 * @param unicode
	 *            select JavaScript unicode. Note that the internal Acrobat
	 *            JavaScript engine does not support unicode, so this may or may
	 *            not work for you
	 */
	...
	/**
	 * Use this method to adds a JavaScript action at the document level. When
	 * the document opens, all this JavaScript runs.
	 * 
	 * @param code
	 *            the JavaScript code
	...
	/** @see org.saipdf.text.pdf.interfaces.PdfDocumentActions#setOpenAction(java.lang.String) */
	public void setOpenAction(String name) {
		pdf.setOpenAction(name);
	}

	/** @see org.saipdf.text.pdf.interfaces.PdfDocumentActions#setOpenAction(org.saipdf.text.pdf.PdfAction) */
	public void setOpenAction(PdfAction action) {
		pdf.setOpenAction(action);
	}

	/**
	 * @see org.saipdf.text.pdf.interfaces.PdfDocumentActions#setAdditionalAction(org.saipdf.text.pdf.PdfName,
	 *      org.saipdf.text.pdf.PdfAction)
	 */
	public void setAdditionalAction(PdfName actionType, PdfAction action) throws DocumentException {
		if (!(actionType.equals(DOCUMENT_CLOSE) || actionType.equals(WILL_SAVE) || actionType.equals(DID_SAVE) || actionType.equals(WILL_PRINT) || actionType
				.equals(DID_PRINT))) {
	...
	/** @see org.saipdf.text.pdf.interfaces.PdfAnnotations#getAcroForm() */
	public PdfAcroForm getAcroForm() {
		return pdf.getAcroForm();
	}

	/** @see org.saipdf.text.pdf.interfaces.PdfAnnotations#addAnnotation(org.saipdf.text.pdf.PdfAnnotation) */
	...
	/** @see org.saipdf.text.pdf.interfaces.PdfAnnotations#addCalculationOrder(org.saipdf.text.pdf.PdfFormField) */
	public void addCalculationOrder(PdfFormField annot) {
		pdf.addCalculationOrder(annot);
	}

	/** @see org.saipdf.text.pdf.interfaces.PdfAnnotations#setSigFlags(int) */
	...
	/**
	 * Use this method to creates XMP Metadata based on the metadata in the
	 * PdfDocument.
	 */
	...
	/** @see org.saipdf.text.pdf.interfaces.PdfXConformance#setPDFXConformance(int) */
	...
	/** @see org.saipdf.text.pdf.interfaces.PdfXConformance#getPDFXConformance() */
	public int getPDFXConformance() {
		return pdfxConformance.getPDFXConformance();
	}

	/** @see org.saipdf.text.pdf.interfaces.PdfXConformance#isPdfX() */
	...
	// [C11] Output intents
	/**
	 * Sets the values of the output intent dictionary. Null values are allowed
	 * to suppress any key.
	 * 
	 * @param outputConditionIdentifier
	 *            a value
	 * @param outputCondition
	 *            a value, "PDFA/A" to force GTS_PDFA1, otherwise cued by
	 *            pdfxConformance.
	 * @param registryName
	 *            a value
	 * @param info
	...
	/**
	 * Use this method to copy the output intent dictionary from another
	 ...
	 *            output intent dictionary, <CODE>false</CODE> to insert the
	 *            dictionary if it exists
	 * @throws IOException
	 *             on error
	 * @return <CODE>true</CODE> if the output intent dictionary exists,
	...
	/**
	 * @deprecated As of iText 2.0.7, use {@link #ALLOW_PRINTING} instead.
	 *             Scheduled for removal at or after 2.2.0
	 */
	public static final int AllowPrinting = ALLOW_PRINTING;
	/**
	 * @deprecated As of iText 2.0.7, use {@link #ALLOW_MODIFY_CONTENTS} instead.
	 *             Scheduled for removal at or after 2.2.0
	 */
	public static final int AllowModifyContents = ALLOW_MODIFY_CONTENTS;
	/**
	 * @deprecated As of iText 2.0.7, use {@link #ALLOW_COPY} instead. Scheduled
	 *             for removal at or after 2.2.0
	 */
	public static final int AllowCopy = ALLOW_COPY;
	/**
	 * @deprecated As of iText 2.0.7, use {@link #ALLOW_MODIFY_ANNOTATIONS}
	 *             instead. Scheduled for removal at or after 2.2.0
	 */
	public static final int AllowModifyAnnotations = ALLOW_MODIFY_ANNOTATIONS;
	/**
	 * @deprecated As of iText 2.0.7, use {@link #ALLOW_FILL_IN} instead.
	 *             Scheduled for removal at or after 2.2.0
	 */
	public static final int AllowFillIn = ALLOW_FILL_IN;
	/**
	 * @deprecated As of iText 2.0.7, use {@link #ALLOW_SCREENREADERS} instead.
	 *             Scheduled for removal at or after 2.2.0
	 */
	public static final int AllowScreenReaders = ALLOW_SCREENREADERS;
	/**
	 * @deprecated As of iText 2.0.7, use {@link #ALLOW_ASSEMBLY} instead.
	 *             Scheduled for removal at or after 2.2.0
	 */
	public static final int AllowAssembly = ALLOW_ASSEMBLY;
	/**
	 * @deprecated As of iText 2.0.7, use {@link #ALLOW_DEGRADED_PRINTING}
	 *             instead. Scheduled for removal at or after 2.2.0
	 */
	...
	/**
	 * @deprecated As of iText 2.0.7, use {@link #STANDARD_ENCRYPTION_40}
	 *             instead. Scheduled for removal at or after 2.2.0
	 */
	public static final boolean STRENGTH40BITS = false;
	/**
	 * @deprecated As of iText 2.0.7, use {@link #STANDARD_ENCRYPTION_128}
	 *             instead. Scheduled for removal at or after 2.2.0
	 */
	...
	/**
	 * @see org.saipdf.text.pdf.interfaces.PdfEncryptionSettings#setEncryption(byte[],
	 *      byte[], int, int)
	 */
	...
	/**
	 * @see org.saipdf.text.pdf.interfaces.PdfEncryptionSettings#setEncryption(java.security.cert.Certificate[],
	 *      int[], int)
	 */
	 ...
	 * @deprecated As of iText 2.0.3, replaced by (@link #setEncryption(byte[],
	 *             byte[], int, int)}. Scheduled for removal at or after 2.2.0
	 */
	...
	/**
	 * Use this method to set the document's compression to the PDF 1.5 mode
	 * with object streams and xref streams. It can be set at any time but once
	 * set it can't be unset.
	 * <p>
	 * If set before opening the document it will also set the pdf version to
	 * 1.5.
	 */
	...
	/**
	 * Adds a <CODE>BaseFont</CODE> to the document but not to the page
	 * resources. It is used for templates.
	 * 
	 * @param bf
	 *            the <CODE>BaseFont</CODE> to add
	 * @return an <CODE>Object[]</CODE> where position 0 is a
	 *         <CODE>PdfName</CODE> and position 1 is an
	...
	/**
	 * The form XObjects in this document. The key is the xref and the value is
	 * Object[]{PdfName, template}.
	 */
	 ...
	 *            the template to add
	 * @param forcedName
	 *            the template name, rather than a generated one. Can be null
		...
		try {
			if (obj == null) {
				...
				if (template.getType() == PdfTemplate.TYPE_IMPORTED) {
					// If we got here from PdfCopy we'll have to fill
					// importedPages
	...
	/**
	 * Use this method to releases the memory used by a template. This method
	 * writes the template to the output. The template can still be added to any
	 * content but changes to the template itself won't have any effect.
	 * 
	 * @param tp
	...
	/**
	 * Use this method to get a page from other PDF document. The page can be
	 * used as any other PdfTemplate. Note that calling this method more than
	...
	/**
	 * Use this method to writes the reader to the document and free the memory
	 * used by it. The main use is when concatenating multiple documents to keep
	 * the memory usage restricted to the current appending document.
	 * 
	 * @param reader
	 *            the <CODE>PdfReader</CODE> to free
	 * @throws IOException
	 *             on error
	 */
	...
	/**
	 * Use this method to gets the current document size. This size only
	 * includes the data already written to the output stream, it does not
	 * include templates or fonts. It is useful if used with
	 * <CODE>freeReader()</CODE> when concatenating many documents and an idea
	 * of the current size is needed.
	 * 
	...
	/**
	 * Adds a <CODE>SpotColor</CODE> to the document but not to the page
	 * resources.
	 * 
	 * @param spc
	 *            the <CODE>SpotColor</CODE> to add
	 * @return an <CODE>Object[]</CODE> where position 0 is a
	 *         <CODE>PdfName</CODE> and position 1 is an
	 *         <CODE>PdfIndirectReference</CODE>
	 */
	...
	/**
	 * Gets the structure tree root. If the document is not marked for tagging
	 ...
	 * Each call fills the dictionary with the current layer state. It's
	 * advisable to only call this method right before close and do any
	...
	/**
	 * Use this method to set a collection of optional content groups whose
	 * states are intended to follow a "radio button" paradigm. That is, the
	 * state of at most one optional content group in the array should be ON at
	 * a time: if one group is turned ON, all others must be turned OFF.
	 * 
	 * @param group
	 *            the radio group
	 */
	...
	/**
	 * Use this method to lock an optional content group. The state of a locked
	 * group cannot be changed through the user interface of a viewer
	 * application. Producers can use this entry to prevent the visibility of
	...
	/**
	 * Use this method to set the crop box. The crop box should not be rotated
	 * even if the page is rotated. This change only takes effect in the next
	 * page.
	 * 
	 * @param crop
	 *            the crop box
	...
	/**
	 * Use this method to set the page box sizes. Allowed names are: "crop",
	 * "trim", "art" and "bleed".
	 * 
	 * @param boxName
	 *            the box size
	...
	/**
	 * Use this method to get the size of a trim, art, crop or bleed box, or
	 * null if not defined.
	 * 
	 * @param boxName
	...
	/**
	 * Use this method to make sure a page is added, even if it's empty. If you
	 * use setPageEmpty(false), invoking newPage() after a blank page will add a
	 * newPage. setPageEmpty(true) won't have any effect.
	 * 
	 * @param pageEmpty
	 *            the state
	 */
	...
	/**
	 * @see org.saipdf.text.pdf.interfaces.PdfPageActions#setPageAction(org.saipdf.text.pdf.PdfName,
	 *      org.saipdf.text.pdf.PdfAction)
	 */
	...
	/** @see org.saipdf.text.pdf.interfaces.PdfPageActions#setDuration(int) */
	public void setDuration(int seconds) {
		pdf.setDuration(seconds);
	}

	/** @see org.saipdf.text.pdf.interfaces.PdfPageActions#setTransition(org.saipdf.text.pdf.PdfTransition) */
	...
	/**
	 * A group attributes dictionary specifying the attributes of the page's
	 ...
	 * Extra word spacing will grow <CODE>ratio</CODE> times more than extra
	 * character spacing.
	 */
	private float spaceCharRatio = SPACE_CHAR_RATIO_DEFAULT;

	/**
	 * Use this method to gets the space/character extra spacing ratio for fully
	 * justified text.
	 * 
	...
	/**
	 * Use this method to set the ratio between the extra word spacing and the
	 * extra character spacing when the text is fully justified. Extra word
	 * spacing will grow <CODE>spaceCharRatio</CODE> times more than extra
	 * character spacing. If the ratio is
	 * <CODE>PdfWriter.NO_SPACE_CHAR_RATIO</CODE> then the extra character
	 * spacing will be zero.
	 * 
	 * @param spaceCharRatio
	 *            the ratio between the extra word spacing and the extra
	 *            character spacing
	 */
	...
	/**
	 * Use bidirectional reordering with left-to-right preferential run
	 * direction.
	 */
	public static final int RUN_DIRECTION_LTR = 2;
	/**
	 * Use bidirectional reordering with right-to-left preferential run
	 * direction.
	 */
	...
	/**
	 * Use this method to set the run direction. This is only used as a
	 * placeholder as it does not affect anything.
	 * 
	 * @param runDirection
	 *            the run direction
	 */
	...
	/**
	 * Use this method to get the user unit. A user unit is a value that defines
	 * the default user space unit. The minimum UserUnit is 1 (1 unit = 1/72
	 * inch). The maximum UserUnit is 75,000. Note that this userunit only works
	...
	/**
	 * Use this method to set the user unit. A UserUnit is a value that defines
	 * the default user space unit. The minimum UserUnit is 1 (1 unit = 1/72
	 * inch). The maximum UserUnit is 75,000. Note that this userunit only works
	 * starting with PDF1.6!
	 * 
	...
	/**
	 * Use this method to sets the default colorspace that will be applied to
	 * all the document. The colorspace is only applied if another colorspace
	 * with the same name is not present in the content.
	 * <p>
	 * The colorspace is applied immediately when creating templates and at the
	 * page end for the main document content.
	 * 
	 * @param key
	 *            the name of the colorspace. It can be
	 *            <CODE>PdfName.DEFAULTGRAY</CODE>,
	 *            <CODE>PdfName.DEFAULTRGB</CODE> or
	 *            <CODE>PdfName.DEFAULTCMYK</CODE>
	 * @param cs
	 *            the colorspace. A <CODE>null</CODE> or <CODE>PdfNull</CODE>
	...
	/**
	 * Use this method to set the image sequence, so that it follows the text in
	 * strict order (or not).
	 * 
	 * @param strictImageSequence
	...
	/**
	 * Use this method to adds an image to the document but not to the page
	 * resources. It is used with templates and <CODE>Document.add(Image)</CODE>
	 * . Use this method only if you know what you're doing!
	 * 
	 * @param image
	 *            the <CODE>Image</CODE> to add
	...
	/**
	 * Adds an image to the document but not to the page resources. It is used
	 * with templates and <CODE>Document.add(Image)</CODE>. Use this method only
	 * if you know what you're doing!
	 * 
	 * @param image
	 *            the <CODE>Image</CODE> to add
	 * @param fixedRef
	 *            the reference to used. It may be <CODE>null</CODE>, a
	 *            <CODE>PdfIndirectReference</CODE> or a
	...
	/**
	 * Gets an indirect reference to a JBIG2 Globals stream. Adds the stream if
	 * it hasn't already been added to the writer.
	 * 
	 * @param content
	 *            a byte array that may already been added to the writer inside
	 *            a stream object.
	 * @since 2.1.5
	 */
	...
	/**
	 * Checks if a <CODE>Table</CODE> fits the current page of the
	 ...
	 * @param margin
	 *            a certain margin
	 * @return <CODE>true</CODE> if the <CODE>Table</CODE> fits the page,
	 *         <CODE>false</CODE> otherwise.
	 */
	...
	/**
	 * Gets the flag indicating the presence of structure elements that contain
	 ...
	 * 
	 * @return <code>true</code> if the transparency blending colorspace is RGB,
	 *         <code>false</code> if it is the default blending colorspace
	 * @since 2.1.0
	 */
	 ...
	 * expected. The RGB blending will be applied to all subsequent pages until
	 * other value is set. Note that this is a generic solution that may not
	 * work in all cases.
	 * 
	 * @param rgbTransparencyBlending
	 *            <code>true</code> to set the transparency blending colorspace
	 *            to RGB, <code>false</code> to use the default blending
	 *            colorspace
	 * @since 2.1.0
	 */
