 ...
 * <P>
 * When using variable borders (
 * {@link org.saipdf.text.Rectangle#isUseVariableBorders
 * isUseVariableBorders()} == true), the borders are drawn completely inside the
 * cell Rectangle so that adjacent cell borders will not overlap. Otherwise, the
 * borders are drawn on top of the edges of the cell Rectangle and will overlap
 * the borders of adjacent cells.
 * 
 * @see org.saipdf.text.Rectangle
 * @see org.saipdf.text.Cell
 * @see PdfLine
 * @see PdfTable
 */
...
public class PdfCell extends Rectangle {
	...
	/**
	 * This is the total height of the content of the cell. Note that the actual
	 * cell height may be larger due to another cell on the row *
	 */
	private float contentHeight = 0.0f;

	/**
	 * Indicates that the largest ascender height should be used to determine
	 * the height of the first line. Setting this to true can help with vertical
	 * alignment problems.
	 */
	 ...
	 * of the last line (so characters like y don't dip into the border).
	 */
	 ...
	 * 
	 * @param cell
	 *            the original <CODE>Cell</CODE>
	 * @param rownumber
	 *            the number of the <CODE>Row</CODE> the <CODE>Cell</CODE> was
	 *            in.
	 * @param left
	 *            the left border of the <CODE>PdfCell</CODE>
	 * @param right
	 *            the right border of the <CODE>PdfCell</CODE>
	 * @param top
	 *            the top border of the <CODE>PdfCell</CODE>
	 * @param cellspacing
	 *            the cellspacing of the <CODE>Table</CODE>
	 * @param cellpadding
	 *            the cellpadding of the <CODE>Table</CODE>
	 */
	...
	public PdfCell(Cell cell, int rownumber, float left, float right, float top, float cellspacing, float cellpadding) {
		...
		for (Iterator i = cell.getElements(); i.hasNext();) {
			...
			default:
				...
				if (chunks.isEmpty()) {
					addLine(line); // add empty line - all cells need some lines
				...
				// if the element is a paragraph, section or chapter, we reset
				// the alignment and add the line
				switch (element.type()) {
				...
				if (more != null && more.length() > 0) {
					...
					if (lastLine.size() >= 0) {
						...
						while (lastChunk.toString().length() > 0 && lastChunk.width() + moreWidth > right - left) {
							// Remove characters to leave room for the 'more'
							// indicator
	...
	/**
	 * Sets the bottom of the Rectangle and determines the proper {link
	 * #verticalOffset} to appropriately align the contents vertically.
	 * 
	 * @param value
	 */
	public void setBottom(float value) {
		...
		float totalHeight = ury - value; // can't use top (already compensates
											// for cellspacing)
	...
	/**
	 * Calculates what the height of the first line should be so that the
	 * content will be flush with the top. For text, this is the height of the
	 * ascender. For an image, it is the actual height of the image.
	 * 
	...
	/**
	 * Gets the amount of the border for the specified side that is inside the
	 * Rectangle. For non-variable width borders this is only 1/2 the border
	 * width on that side. This always returns 0 if {@link #useBorderPadding} is
	 * false;
	 * 
	 * @param side
	 *            the side to check. One of the side constants in
	 *            {@link org.saipdf.text.Rectangle}
	...
	private float getBorderWidthInside(int side) {
		...
		if (useBorderPadding) {
			...
			// non-variable (original style) borders overlap the rectangle (only
			// 1/2 counts)
	 ...
	 *            the right border
	 * @param extraHeight
	 *            extra height to add above image
	 * @param alignment
	 *            horizontal alignment (constant from Element class)
	 ...
	 * 
	 * @param top
	 *            the top of the part of the table that can be drawn
	 * @param bottom
	...
	public ArrayList getLines(float top, float bottom) {
		...
		// if the bottom of the page is higher than the top of the cell: do
		// nothing
		if (getTop() < bottom) {
			return result;
		}
		...
		for (int i = 0; i < size && aboveBottom; i++) {
			...
			// if the currentPosition is higher than the bottom, we add the line
			// to the result
		...
		// if the bottom of the cell is higher than the bottom of the page, the
		// cell is written, so we can remove all lines
		float difference = 0f;
		if (!header) {
			if (aboveBottom) {
				lines = new ArrayList();
	 ...
	 * 
	 * @return <CODE>true</CODE> if all the lines are already drawn;
	 ...
	 * 
	 * @param element
	 *            an element in the cell
	 * @param action
	 *            an action that should be coupled to the cell
	 * @param allActions
	 */
