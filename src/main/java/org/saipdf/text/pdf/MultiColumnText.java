...
import org.saipdf.text.ElementListener;
...
/**
 * Formats content into one or more columns bounded by a rectangle. The columns
 * may be simple rectangles or more complicated shapes. Add all of the columns
 * before adding content. Column continuation is supported. A MultiColumnText
...
public class MultiColumnText implements Element {
	...
	/**
	 * total desiredHeight of columns. If <CODE>AUTOMATIC</CODE>, this means
	 * fill pages until done. This may be larger than one page
	 */
	...
	/**
	 * Top of the columns - y position on starting page. If
	 * <CODE>AUTOMATIC</CODE>, it means current y position when added to
	 * document
	 */
	private float top;

	/**
	 * ColumnText object used to do all the real work. This same object is used
	 * for all columns
	 */
	...
	/**
	 * Default constructor. Sets height to <CODE>AUTOMATIC</CODE>. Columns will
	 * repeat on each page as necessary to accommodate content length.
	 */
	...
	/**
	 * Construct a MultiColumnText container of the specified height. If height
	 * is <CODE>AUTOMATIC</CODE>, fill complete pages until done. If a specific
	 * height is used, it may span one or more pages.
	 * 
	 * @param height
	 */
	...
	/**
	 * Construct a MultiColumnText container of the specified height starting at
	...
	/**
	 * Indicates that all of the text did not fit in the specified height. Note
	 * that isOverflow will return false before the MultiColumnText object has
	 * been added to the document. It will always be false if the height is
	 * AUTOMATIC.
	 * 
	 ...
	 * Parameters like <CODE>setArabicOptions</CODE> must be set in this way.
	 * 
	 * @param sourceColumn
	 */
	...
	/**
	 * Add a new column. The parameters are limits for each column wall in the
	 * format of a sequence of points (x1,y1,x2,y2,...).
	 * 
	 * @param left
	...
	/**
	 * Add a simple rectangular column with specified left and right x position
	 * boundaries.
	 * 
	 * @param left
	 *            left boundary
	...
	/**
	 * Add the specified number of evenly spaced rectangular columns. Columns
	 * will be separated by the specified gutterWidth.
	 * 
	 * @param left
	...
	/**
	 * Add an element to be rendered in a column. Note that you can only add a
	 * <CODE>Phrase</CODE> or a <CODE>Chunk</CODE> if the columns are not all
	 * simple. This is an underlying restriction in
	 * {@link org.saipdf.text.pdf.ColumnText}
	 * 
	 * @param element
	 *            element to add
	 * @throws DocumentException
	 *             if element can't be added
	 */
	...
	/**
	 * Write out the columns. After writing, use {@link #isOverflow()} to see if
	 * all text was written.
	 * 
	 * @param canvas
	...
	public float write(PdfContentByte canvas, PdfDocument document, float documentY) throws DocumentException {
		...
		try {
			while (!done) {
				if (top == AUTOMATIC) {
					top = document.getVerticalPosition(true); // RS - 07/07/2005
																// - Get current
																// doc writing
																// position for
																// top of
					...
					nextY = document.getVerticalPosition(true); // RS -
																// 07/07/2005 -
																// - Get current
																// doc writing
																// position for
	...
	/**
	 * @see org.saipdf.text.Element#isContent()
	 * @since iText 2.0.8
	 */
	...
	/**
	 * Calculates the appropriate y position for the bottom of the columns on
	 * this page.
	 * 
	 ...
	 * issuing a page break if needed.
	 * 
	 * @throws DocumentException
	 *             on error
	 */
	...
	private class ColumnDef {
		...
		/**
		 * Resolves the positions for the specified side of the column into real
		 * numbers once the top of the column is known.
		 * 
		 * @param side
		 *            either <CODE>Rectangle.LEFT</CODE> or
		 *            <CODE>Rectangle.RIGHT</CODE>
