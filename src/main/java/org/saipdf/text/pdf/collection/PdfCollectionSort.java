...
import org.saipdf.text.pdf.PdfBoolean;
...
public class PdfCollectionSort extends PdfDictionary {
	 ...
	 * 
	 * @param ascending
	 *            an array with every element corresponding with a name of a
	 *            field.
	 */
	public void setSortOrder(boolean[] ascending) {
		PdfObject o = get(PdfName.S);
		if (o instanceof PdfArray) {
			if (((PdfArray) o).size() != ascending.length) {
				throw new IllegalArgumentException(
						MessageLocalization.getComposedMessage("the.number.of.booleans.in.this.array.doesn.t.correspond.with.the.number.of.fields"));
			}
			PdfArray array = new PdfArray();
