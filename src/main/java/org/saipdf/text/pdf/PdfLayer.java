 ...
 * graphics that can be made visible or invisible dynamically by users of viewer
 * applications. In iText they are referenced as layers.
 * 
...
public class PdfLayer extends PdfDictionary implements PdfOCG {
	...
	/**
	 * Creates a title layer. A title layer is not really a layer but a
	 * collection of layers under the same title heading.
	 * 
	 * @param title
	 *            the title text
	 * @param writer
	...
	public void addChild(PdfLayer child) {
		if (child.parent != null)
			throw new IllegalArgumentException(MessageLocalization.getComposedMessage("the.layer.1.already.has.a.parent",
					((PdfString) child.get(PdfName.NAME)).toUnicodeString()));
		child.parent = this;
		if (children == null)
	 ...
	 * 
	 * @return the children layers or <CODE>null</CODE> if the layer has no
	 *         children
	 */
	 ...
	 * 
	 * @param ref
	 *            The reference to the OCG object
	 * @since 2.1.2
	 */
	...
	/**
	 * Gets the dictionary representing the layer. It just returns
	 * <CODE>this</CODE>.
	 * 
	...
	/**
	 * Used by the creating application to store application-specific data
	 * associated with this optional content group.
	 * 
	 * @param creator
	 *            a text string specifying the application that created the
	 *            group
	 * @param subtype
	 *            a string defining the type of content controlled by the group.
	 *            Suggested values include but are not limited to
	 *            <B>Artwork</B>, for graphic-design or publishing applications,
	 *            and <B>Technical</B>, for technical designs such as building
	 *            plans or schematics
	 */
	...
	/**
	 * Specifies the language of the content controlled by this optional content
	 * group
	 * 
	 * @param lang
	 *            a language string which specifies a language and possibly a
	 *            locale (for example, <B>es-MX</B> represents Mexican Spanish)
	 * @param preferred
	 *            used by viewer applications when there is a partial match but
	 *            no exact match between the system language and the language
	 *            strings in all usage dictionaries
	 */
	...
	/**
	 * Specifies the recommended state for content in this group when the
	 * document (or part of it) is saved by a viewer application to a format
	 * that does not support optional content (for example, an earlier version
	 * of PDF or a raster image format).
	 * 
	 * @param export
	 *            the export state
	 */
	...
	/**
	 * Specifies a range of magnifications at which the content in this optional
	 * content group is best viewed.
	 * 
	 * @param min
	 *            the minimum recommended magnification factors at which the
	 *            group should be ON. A negative value will set the default to 0
	 * @param max
	 *            the maximum recommended magnification factor at which the
	 *            group should be ON. A negative value will set the largest
	 *            possible magnification supported by the viewer application
	 */
	 ...
	 *            for example, <B>Trapping</B>, <B>PrintersMarks</B> and
	 *            <B>Watermark</B>
	 * @param printstate
	 *            indicates that the group should be set to that state when the
	 *            document is printed from a viewer application
	 */
	...
	/**
	 * Indicates that the group should be set to that state when the document is
	...
	/**
	 * Sets the visibility of the layer in Acrobat's layer panel. If
	 * <CODE>false</CODE> the layer cannot be directly manipulated by the user.
