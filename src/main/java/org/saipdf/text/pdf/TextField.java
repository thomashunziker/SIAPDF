...
/**
 * Supports text, combo and list fields generating the correct appearances. All
 * the option in the Acrobat GUI are supported in an easy to use API.
 * 
...
public class TextField extends BaseField {
	...
	/**
	 * Obfuscates a password <code>String</code>. Every character is replaced by
	 * an asterisk (*).
	 * 
	 * @param text
	 * @return String
	...
	PdfAppearance getListAppearance() throws IOException, DocumentException {
		...
		for (int curVal = 0; curVal < choiceSelections.size(); ++curVal) {
			...
			// only draw selections within our display range... not strictly
			// necessary with
			// that clipping rect from above, but it certainly doesn't hurt
			// either
		...
		for (int idx = first; idx < last; ++idx, yp -= leading) {
			...
			// highlight selected values against their (presumably) darker
			// background
	...
	/**
	 * Sets the export values in list/combo fields. If this array is
	 * <CODE>null</CODE> then the choice values will also be used as the export
	 * values.
	 * 
	 * @param choiceExports
	...
	/**
	 * adds another (or a first I suppose) selection to a MULTISELECT list. This
	 * doesn't do anything unless this.options & MUTLISELECT != 0
	 * 
	 * @param selection
	 *            new selection
	 */
	...
	/**
	 * replaces the existing selections with the param. If this field isn't a
	 * MULTISELECT list, all but the first element will be removed.
	 * 
	 * @param selections
