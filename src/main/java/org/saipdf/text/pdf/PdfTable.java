...
/**
 * <CODE>PdfTable</CODE> is an object that contains the graphics and text of a
 * table.
 * 
 * @see org.saipdf.text.Table
 * @see org.saipdf.text.Row
...
public class PdfTable extends Rectangle {
	 ...
	 *            a <CODE>Table</CODE>
	 * @param left
	 *            the left border on the page
	 * @param right
	 *            the right border on the page
	 * @param top
	 *            the start position of the top of the table
	...
	/**
	 * Updates the table row additions in the underlying table object and
	 * deletes all table rows, in order to preserve memory and detect future row
	 * additions.
	 * <p>
	 * <b>Pre-requisite</b>: the object must have been built with the parameter
		...
		for (Iterator rowIterator = table.iterator(); rowIterator.hasNext();) {
			...
			if (row.isEmpty()) {
				...
				for (int i = 0; i < row.getColumns(); i++) {
					...
					if (cell != null) {
						currentCell = new PdfCell(cell, rowNumber + prevRows, positions[i], positions[i + cell.getColspan()], offsets[rowNumber],
	...
	/** @see org.saipdf.text.Element#type() */
	public int type() {
