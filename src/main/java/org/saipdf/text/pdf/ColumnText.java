...
public class ColumnText {
	 ...
	 * 
	 * @param canvas
	 *            the place where the text will be written to. Can be a
	 *            template.
	 */
	 ...
	 * 
	 * @param org
	 *            the original <CODE>ColumnText</CODE>
	 ...
	 * 
	 * @param simulate
	 *            <CODE>true</CODE> to simulate the writing to the document
	 * @return returns the result of the operation. It can be
	 *         <CODE>NO_MORE_TEXT</CODE> and/or <CODE>NO_MORE_COLUMN</CODE>
	 * @throws DocumentException
	 *             on error
	 ...
	 * 
	 * @param phrase
	 *            the <CODE>Phrase</CODE> containing the line
	 * @param runDirection
	 *            the run direction
	 * @param arabicOptions
	 ...
	 *            the options for the arabic shaping
	 */
	public static void showTextAligned(PdfContentByte canvas, int alignment, Phrase phrase, float x, float y, float rotation, int runDirection,
			int arabicOptions) {
	...
	protected int goComposite(boolean simulate) throws DocumentException {
		...
		main_loop: while (true) {
			...
			if (element.type() == Element.PARAGRAPH) {
				...
				org.saipdf.text.List list = (org.saipdf.text.List) element;
				...
				for (int k = 0; k < items.size(); ++k) {
					...
					if (obj instanceof ListItem) {
						if (count == listIdx) {
							...
							++count;
					} else if (obj instanceof org.saipdf.text.List) {
						stack.push(new Object[] { list, new Integer(k), new Float(listIndentation) });
						list = (org.saipdf.text.List) obj;
						items = list.getItems();
					...
					if (k == items.size() - 1) {
						if (!stack.isEmpty()) {
							...
							list = (org.saipdf.text.List) objs[0];
							items = list.getItems();
				...
				// don't write anything in the current column if there's no more
				// space available
				if (yLine < minY || yLine > maxY)
					return NO_MORE_COLUMN;
				...
				// how many header rows are real header rows; how many are
				// footer rows?
				...
				if (!table.isComplete())
					yTemp += footerHeight;
				// either k is the first row that doesn't fit on the page
				// (break);
				if (k < table.size()) {
				...
				if (!simulate) {
					...
					// if k < table.size(), we must indicate that the new table
					// is complete;
					// otherwise no footers will be added (because iText thinks
					// the table continues on the same page)
					...
					// we add the footer rows if necessary (not for incomplete
					// tables)
	 ...
	 * 
	 * @param adjustFirstLine
	 *            <CODE>true</CODE> to adjust the first line, <CODE>false</CODE>
	 *            otherwise
	 */
