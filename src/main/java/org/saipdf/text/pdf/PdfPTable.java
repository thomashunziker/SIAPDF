...
import org.saipdf.text.LargeElement;
import org.saipdf.text.Phrase;
import org.saipdf.text.Rectangle;
import org.saipdf.text.error_messages.MessageLocalization;
import org.saipdf.text.pdf.events.PdfPTableEventForwarder;


/**
 * This is a table that can be put at an absolute position but can also be added
 * to the document as the class <CODE>Table</CODE>. In the last case when
 * crossing pages the table always break at full rows; if a row is bigger than
...
public class PdfPTable implements LargeElement {
	 ...
	 * 
	 * @param table
	 *            the <CODE>PdfPTable</CODE> to be copied
	 */
	 ...
	 *             if the number of widths is different than the number of
	 *             columns
	 */
	 ...
	 * 
	 * @param firsttime
	 *            if true, the heights of the rows will be recalculated. This
	 *            takes time; normally the heights of the rows are already
	 *            calcultated, so in most cases, it's save to use false as
	 *            parameter.
	 * @return the total height of the table. Note that it will be 0 if you
	 *         didn't specify the width of the table with setTotalWidth().
	 ...
	 * 
	 * @param image
	 *            the <CODE>Image</CODE> to add to the table. This image will
	 *            fit in the cell
	 */
	 ...
	 * @see #beginWritingRows(org.saipdf.text.pdf.PdfContentByte)
	 */
	 ...
	 * @param colEnd
	 *            the last column to be written + 1. If it is -1 all the columns
	 *            to the end are written
	 * @param rowStart
	 *            the first row to be written, zero index
	 * @param rowEnd
	 *            the last row to be written + 1. If it is -1 all the rows to
	 ...
	 *            the y write coordinate
	 * @param canvas
	 *            the <CODE>PdfContentByte</CODE> where the rows will be written
	 *            to
	 * @return the y coordinate position of the bottom of the last row
	 */
	 ...
	 * 
	 * @param canvases
	 *            the array returned by <CODE>beginWritingRows()</CODE>
	 */
	 ...
	 * 
	 * @param runDirection
	 *            One of the following values: PdfWriter.RUN_DIRECTION_DEFAULT,
	 *            PdfWriter.RUN_DIRECTION_NO_BIDI, PdfWriter.RUN_DIRECTION_LTR
	 ...
	 * @param extendFinalRow
	 *            false if you don't want to extend the final row of the
	 *            complete table
	 * @since iText 5.0.0
	 */
