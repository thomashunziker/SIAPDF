...
public class PdfSignatureAppearance {
	...
	public void setVisibleSignature(String fieldName) {
		...
		case 90:
			...
			break;
		case 180:
			pageRect = new Rectangle(pageSize.getRight() - pageRect.getLeft(), pageSize.getTop() - pageRect.getBottom(), pageSize.getRight()
					- pageRect.getRight(), pageSize.getTop() - pageRect.getTop());
			break;
		case 270:
			pageRect = new Rectangle(pageSize.getRight() - pageRect.getBottom(), pageRect.getLeft(), pageSize.getRight() - pageRect.getTop(),
					pageRect.getRight());
			break;
		}
		if (rotation != 0)
	...
	public PdfTemplate getAppearance() throws DocumentException {
		...
		if (app[2] == null) {
			...
			if (render == SignatureRenderNameAndDescription) {
				...
				ct2.setSimpleColumn(new Phrase(signedBy, font), signatureRect.getLeft(), signatureRect.getBottom(), signatureRect.getRight(),
						signatureRect.getTop(), signedSize, Element.ALIGN_LEFT);

				ct2.go();
			} else if (render == SignatureRenderGraphicAndDescription) {
				ColumnText ct2 = new ColumnText(t);
				ct2.setRunDirection(runDirection);
				ct2.setSimpleColumn(signatureRect.getLeft(), signatureRect.getBottom(), signatureRect.getRight(), signatureRect.getTop(), 0,
						Element.ALIGN_RIGHT);
				...
				// must calculate the point to draw from to make image appear in
				// middle of column
				float x = 0;
			...
			ct.setSimpleColumn(new Phrase(text, font), dataRect.getLeft(), dataRect.getBottom(), dataRect.getRight(), dataRect.getTop(), size,
					Element.ALIGN_LEFT);
			ct.go();
		}
	 ...
	 */
	public org.saipdf.text.pdf.PdfName getFilter() {
		return filter;
	}
	 ...
	 *         signature in the page
	 */
	public org.saipdf.text.Rectangle getPageRect() {
		return pageRect;
	}
	...
	org.saipdf.text.pdf.ByteBuffer getSigout() {
		return sigout;
	}

	void setSigout(org.saipdf.text.pdf.ByteBuffer sigout) {
		this.sigout = sigout;
	}
		...
		try {
			...
			for (Iterator it = update.getKeys().iterator(); it.hasNext();) {
				...
				if (bf.size() > lit.getPosLength())
					throw new IllegalArgumentException(MessageLocalization.getComposedMessage("the.key.1.is.too.big.is.2.reserved.3", key.toString(),
	 ...
	 */
	public org.saipdf.text.pdf.PdfDictionary getCryptoDictionary() {
	 ...
	 */
	public void setCryptoDictionary(org.saipdf.text.pdf.PdfDictionary cryptoDictionary) {
	 ...
	 */
	public org.saipdf.text.pdf.PdfStamper getStamper() {
		return stamper;
	}

	void setStamper(org.saipdf.text.pdf.PdfStamper stamper) {
	 ...
	 */
	public org.saipdf.text.pdf.PdfSigGenericPKCS getSigStandard() {
		return sigStandard;
	}
