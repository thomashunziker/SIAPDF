...
/**
 * This class captures an AcroForm on input. Basically, it extends Dictionary by
 * indexing the fields of an AcroForm
 * 
...
public class PRAcroForm extends PdfDictionary {
	...
	protected PdfDictionary mergeAttrib(PdfDictionary parent, PdfDictionary child) {
		...
		for (Iterator it = child.getKeys().iterator(); it.hasNext();) {
			...
			if (key.equals(PdfName.DR) || key.equals(PdfName.DA) || key.equals(PdfName.Q) || key.equals(PdfName.FF) || key.equals(PdfName.DV)
					|| key.equals(PdfName.V) || key.equals(PdfName.FT) || key.equals(PdfName.F)) {
