...
import org.saipdf.text.SplitCharacter;
 ...
 * 
 * @see PdfString
 * @see org.saipdf.text.Chunk
 * @see org.saipdf.text.Font
 */
...
public class PdfChunk {
	 ...
	 * Metric attributes.
	 * <P>
	 * This attributes require the measurement of characters widths when
	 * rendering such as underline.
	 */
	 ...
	 * <P>
	 * This attributes do not require the measurement of characters widths when
	 * rendering such as Color.
	 */
	...
	/**
	 * Indicates if the height and offset of the Image has to be taken into
	 * account
	 */
	 ...
	 * 
	 * @param string
	 *            the content of the <CODE>PdfChunk</CODE>-object
	 * @param other
	 ...
	 * 
	 * @param chunk
	 *            the original <CODE>Chunk</CODE>-object
	 * @param action
	 *            the <CODE>PdfAction</CODE> if the <CODE>Chunk</CODE> comes
	 *            from an <CODE>Anchor</CODE>
	 */
	...
	PdfChunk split(float width) {
		...
		if (ft.getFontType() == BaseFont.FONT_TYPE_CJK && ft.getUnicodeEquivalent(' ') != ' ') {
			while (currentPosition < length) {
				...
				if (currentWidth > width)
					break;
				// if a split-character is encountered, the splitPosition is
				// altered
		...
		// if all the characters fit in the total width, null is returned (there
		// is no overflow)
	...
	/**
	 * Truncates this <CODE>PdfChunk</CODE> if it's too long for the given
	 * width.
	 * <P>
	 * Returns <VAR>null</VAR> if the <CODE>PdfChunk</CODE> wasn't truncated.
	 * 
	...
	PdfChunk truncate(float width) {
		...
		// it's no use trying to split if there isn't even enough place for a
		// space
		if (width < font.width()) {
	 ...
	 * 
	 * @return <CODE>true</CODE> if the <CODE>PdfChunk</CODE> split was caused
	 *         by a newline.
	 */
	...
	/**
	 * Gets the width of the <CODE>PdfChunk</CODE> taking into account the extra
	 * character and word spacing.
	 * 
	 * @param charSpacing
	 *            the extra character spacing
	 * @param wordSpacing
	...
	/**
	 * Gets an attribute. The search is made in <CODE>attributes</CODE> and
	 * <CODE>noStroke</CODE>.
	 * 
	 * @param name
	 *            the attribute key
	 ...
	 * 
	 * @return <CODE>true</CODE> if this <CODE>PdfChunk</CODE> needs some
	 *         special metrics handling.
	 */
	...
	/**
	 * Removes all the <VAR>' '</VAR> and <VAR>'-'</VAR>-characters on the right
	 * of a <CODE>String</CODE>.
	 * <P>
	 * 
	 * @param string
	 *            the <CODE>String<CODE> that has to be trimmed.
