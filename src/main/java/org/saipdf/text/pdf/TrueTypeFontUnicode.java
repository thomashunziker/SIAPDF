...
/**
 * Represents a True Type font with Unicode encoding. All the character in the
 * font can be used directly by using the encoding Identity-H or Identity-V.
 * This is the only way to represent some character sets such as Thai.
 * 
...
class TrueTypeFontUnicode extends TrueTypeFont implements Comparator {
	...
	TrueTypeFontUnicode(String ttFile, String enc, boolean emb, byte ttfAfm[], boolean forceRead) throws DocumentException, IOException {
		...
		if ((fileName.toLowerCase().endsWith(".ttf") || fileName.toLowerCase().endsWith(".otf") || fileName.toLowerCase().endsWith(".ttc"))
			...
			// throw new
			// DocumentException(MessageLocalization.getComposedMessage("1.2.does.not.contain.an.usable.cmap",
			// fileName, style));
			if (fontSpecific) {
				fontSpecific = false;
	 ...
	 * 
	 * @param metrics
	 *            metrics[0] contains the glyph index and metrics[2] contains
	 *            the Unicode code
