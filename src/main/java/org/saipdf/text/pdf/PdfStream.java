 ...
 * can read a small portion of a stream at a time, while a string must be read
 * in its entirety. For this reason, objects with potentially large amounts of
 * data, such as images and page descriptions, are represented as streams.<BR>
 * A stream consists of a dictionary that describes a sequence of characters,
 * followed by the keyword <B>stream</B>, followed by zero or more lines of
 * characters, followed by the keyword <B>endstream</B>.<BR>
 * All streams must be <CODE>PdfIndirectObject</CODE>s. The stream dictionary
 * must be a direct object. The keyword <B>stream</B> that follows the stream
 * dictionary should be followed by a carriage return and linefeed or just a
 * linefeed.<BR>
...
public class PdfStream extends PdfDictionary {
	 ...
	 * <CODE>InputStream</CODE> is totally consumed but is not closed. The
	 * general usage is:
	 * <p>
	 * 
	 * <pre>
	 ...
	 * @param writer
	 *            the <CODE>PdfWriter</CODE> for this stream
	 */
	 ...
	 * {@link #PdfStream(InputStream,PdfWriter)} is used to create the stream.
	 * 
	 * @throws IOException
	 *             on error
	 ...
	 *            the compression level (0 = best speed, 9 = best compression,
	 *            -1 is default)
	 * @since 2.1.3
	 */
	...
	/**
	 * @see org.saipdf.text.pdf.PdfDictionary#toPdf(org.saipdf.text.pdf.PdfWriter,
