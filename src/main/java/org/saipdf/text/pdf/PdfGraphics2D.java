...
import org.saipdf.text.pdf.internal.PolylineShape;
...
public class PdfGraphics2D extends Graphics2D {
	...
	public void drawString(String s, float x, float y) {
		...
		if (onlyShapes) {
			...
			// drawGlyphVector(this.font.createGlyphVector(getFontRenderContext(),
			// s), x, y);
		} else {
			...
			// Do nothing if the BaseFont is already bold. This test is not
			// foolproof but it will work most of the times.
			if (baseFont.getPostscriptFontName().toLowerCase().indexOf("bold") < 0) {
				// Get the weight of the font so we can detect fonts with a
				// weight
	...
	private boolean drawImage(Image img, Image mask, AffineTransform xform, Color bgColor, ImageObserver obs) {
		...
		try {
			org.saipdf.text.Image image = null;
			if (!convertImagesToJPEG) {
				image = org.saipdf.text.Image.getInstance(img, bgColor);
			} else {
				...
				scaled.flush();
				scaled = null;
				image = org.saipdf.text.Image.getInstance(baos.toByteArray());

			}
			if (mask != null) {
				org.saipdf.text.Image msk = org.saipdf.text.Image.getInstance(mask, null, true);
				msk.makeMask();
	...
	private void setPaint(boolean invert, double xoffset, double yoffset, boolean fill) {
		if (paint instanceof Color) {
			...
			PdfShading shading = PdfShading.simpleAxial(cb.getPdfWriter(), (float) p1.getX(), normalizeY((float) p1.getY()), (float) p2.getX(),
					normalizeY((float) p2.getY()), c1, c2);
			...
			try {
				...
				org.saipdf.text.Image image = org.saipdf.text.Image.getInstance(img, null);
