...
import org.saipdf.text.pdf.fonts.FontsResourceAnchor;
...
class Type1Font extends BaseFont {
	 ...
	 * 'AdobeStandardEncoding' for a font that can be totally encoded according
	 * to the characters names. For all other names the font is treated as
	 * symbolic.
	 */
	...
	/**
	 * Represents the section CharMetrics in the AFM file. Each value of this
	 * array contains a <CODE>Object[4]</CODE> with an Integer, Integer, String
	 * and int[]. This is the code, width, name and char bbox. The key is the
	 * name of the char and also an Integer with the char number.
	 */
	private HashMap CharMetrics = new HashMap();
	/**
	 * Represents the section KernPairs in the AFM file. The key is the name of
	 * the first character and the value is a <CODE>Object[]</CODE> with 2
	 * elements for each kern pair. Position 0 is the name of the second
	 * character and position 1 is the kerning distance. This is repeated for
	 * all the pairs.
	 */
	...
	/**
	 * Types of records in a PFB file. ASCII is 1 and BINARY is 2. They have to
	 * appear in the PFB file in this sequence.
	 */
	 ...
	 * 
	 * @param ttfAfm
	 *            the AFM file if the input is made with a <CODE>byte</CODE>
	 *            array
	 * @param pfb
	 *            the PFB file if the input is made with a <CODE>byte</CODE>
	 *            array
	 * @param afmFile
	 *            the name of one of the 14 built-in fonts or the location of an
	 *            AFM file. The file must end in '.afm'
	 * @param enc
	...
	Type1Font(String afmFile, String enc, boolean emb, byte ttfAfm[], byte pfb[], boolean forceRead) throws DocumentException, IOException {
		...
		if (!encoding.startsWith("#"))
			PdfEncodings.convertToBytes(" ", enc); // check if the encoding
													// exists
		createEncoding();
	}

	/**
	 * Gets the width from the font according to the <CODE>name</CODE> or, if
	 * the <CODE>name</CODE> is null, meaning it is a symbolic font, the char
	 * <CODE>c</CODE>.
	 * 
	 * @param c
	 *            the char if the font is symbolic
	 * @param name
	 *            the glyph name
	...
	/**
	 * Gets the kerning between two Unicode characters. The characters are
	 * converted to names and this names are used to find the kerning pairs in
	 * the <CODE>HashMap</CODE> <CODE>KernPairs</CODE>.
	 * 
	 * @param char1
	 *            the first char
	 * @param char2
		...
		while ((line = rf.readLine()) != null) {
			...
			while (tok.hasMoreTokens()) {
				...
				if (ident.equals("C"))
					...
					B = new int[] { Integer.parseInt(tokc.nextToken()), Integer.parseInt(tokc.nextToken()), Integer.parseInt(tokc.nextToken()),
	...
	/**
	 * If the embedded flag is <CODE>false</CODE> or if the font is one of the
	 * 14 built in types, it returns <CODE>null</CODE>, otherwise the font is
	 * read and output in a PdfStream object.
	 * 
	 ...
	 * 
	 * @param fontStream
	 *            the indirect reference to a PdfStream containing the font or
	 *            <CODE>null</CODE>
	 * @return the PdfDictionary containing the font descriptor or
	 *         <CODE>null</CODE>
	 */
	 ...
	 * @param shortTag
	 *            a 256 bytes long <CODE>byte</CODE> array where each unused
	 *            byte is represented by 0
	 * @param fontDescriptor
	 *            the indirect reference to a PdfDictionary containing the font
