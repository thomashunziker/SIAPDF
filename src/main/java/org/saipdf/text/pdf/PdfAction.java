...
import org.saipdf.text.pdf.collection.PdfTargetDictionary;


/**
 * A <CODE>PdfAction</CODE> defines an action that can be triggered from a PDF
 * file.
 * 
 * @see PdfDictionary
 */
...
public class PdfAction extends PdfDictionary {
	...
	/**
	 * Construct a new <CODE>PdfAction</CODE> of Subtype URI that accepts the x
	 * and y coordinate of the position that was clicked.
	 * 
	 * @param url
	 * @param isMap
	 */
	...
	/**
	 * Implements name actions. The action can be FIRSTPAGE, LASTPAGE, NEXTPAGE,
	 * PREVPAGE and PRINTDIALOG.
	 * 
	 * @param named
	 *            the named action
	 */
	 ...
	 *            application. It can be <CODE>null</CODE>.
	 * @param operation
	 *            (Windows-specific) the operation to perform: "open" - Open a
	 *            document, "print" - Print a document. It can be
	 *            <CODE>null</CODE>.
	 * @param defaultDir
	 *            (Windows-specific) the default directory in standard DOS
	 *            syntax. It can be <CODE>null</CODE>.
	 */
	...
	/**
	 * Creates a JavaScript action. If the JavaScript is smaller than 50
	 * characters it will be placed as a string, otherwise it will be placed as
	 * a compressed stream.
	 * 
	 * @param code
	 *            the JavaScript code
	...
	/**
	 * Creates a JavaScript action. If the JavaScript is smaller than 50
	 * characters it will be place as a string, otherwise it will be placed as a
	 * compressed stream.
	 * 
	 * @param code
	 *            the JavaScript code
	 ...
	 * @param isName
	 *            if true sets the destination as a name, if false sets it as a
	 *            String
	 * @return a GoTo action
	 */
	 ...
	 *            String
	 * @param newWindow
	 *            open the document in a new window if <CODE>true</CODE>, if
	 *            false the current document is replaced by the new document.
	 ...
	 * 
	 * @param filename
	 *            the root document of the target (null if the target is in the
	 *            same document)
	 * @param dest
	 *            the named destination
	 * @param isName
	 ...
	 * @param dest
	 *            the destination inside the target document, can be of type
	 *            PdfDestination, PdfName, or PdfString
	 * @param newWindow
	 *            if true, the destination document should be opened in a new
	 *            window
	 * @return a GoToE action
	 */
	 ...
	 * 
	 * @param state
	 *            an array consisting of any number of sequences beginning with
	 *            a <CODE>PdfName</CODE> or <CODE>String</CODE> (ON, OFF, or
	 *            Toggle) followed by one or more optional content group
	 *            dictionaries <CODE>PdfLayer</CODE> or a
	 *            <CODE>PdfIndirectReference</CODE> to a <CODE>PdfLayer</CODE>.<br>
	 *            The array elements are processed from left to right; each name
	 *            is applied to the subsequent groups until the next name is
	 *            encountered:
	 *            <ul>
	 ...
	 *            </ul>
	 * @param preserveRB
	 *            if <CODE>true</CODE>, indicates that radio-button state
	 *            relationships between optional content groups (as specified by
	 *            the RBGroups entry in the current configuration dictionary)
	 *            should be preserved when the states in the <CODE>state</CODE>
	 *            array are applied. That is, if a group is set to ON (either by
	 *            ON or Toggle) during processing of the <CODE>state</CODE>
	 *            array, any other groups belong to the same radio-button group
	 *            are turned OFF. If a group is set to OFF, there is no effect
	 *            on other groups.<br>
	 *            If <CODE>false</CODE>, radio-button state relationships, if
	 *            any, are ignored
	 * @return the action
	 */
