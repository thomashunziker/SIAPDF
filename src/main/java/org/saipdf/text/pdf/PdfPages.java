 ...
 * <P>
 * The Pages of a document are accessible through a tree of nodes known as the
 * Pages tree. This tree defines the ordering of the pages in the document.<BR>
	...
	int reorderPages(int order[]) throws DocumentException {
		...
		if (parents.size() > 1)
			throw new DocumentException(
					MessageLocalization.getComposedMessage("page.reordering.requires.a.single.parent.in.the.page.tree.call.pdfwriter.setlinearmode.after.open"));
		...
		for (int k = 0; k < max; ++k) {
			int p = order[k];
			if (p < 1 || p > max)
				throw new DocumentException(MessageLocalization.getComposedMessage("page.reordering.requires.pages.between.1.and.1.found.2",
