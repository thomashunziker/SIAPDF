...
public class TiffImage {
	 ...
	 *            the page to get. The first page is 1
	 * @param direct
	 *            for single strip, CCITT images, generate the image by direct
	 *            byte copying. It's faster but may not work every time
	 * @return the <CODE>Image</CODE>
	 */
	public static Image getTiffImage(RandomAccessFileOrArray s, int page, boolean direct) {
		...
		try {
			...
			if ((size == null || (size.length == 1 && (size[0] == 0 || size[0] + offset[0] > s.length()))) && h == rowsStrip) { // some
																																// TIFF
																																// producers
																																// are
																																// really
	...
	protected static Image getTiffImageColor(TIFFDirectory dir, RandomAccessFileOrArray s) {
		try {
			...
			reverse = (fillOrder == TIFFConstants.FILLORDER_LSB2MSB);
			int rowsStrip = h;
			if (dir.isTagPresent(TIFFConstants.TIFFTAG_ROWSPERSTRIP)) // another
																		// hack
																		// for
																		// broken
																		// tiffs
				...
				if (predictorField != null) {
					...
					if (predictor == 2 && bitsPerSample != 8) {
						throw new RuntimeException(MessageLocalization.getComposedMessage(
								"1.bit.samples.are.not.supported.for.horizontal.differencing.predictor", bitsPerSample));
					}
				}
			...
			if (compression == TIFFConstants.COMPRESSION_OJPEG) {
				...
				// Assume that the TIFFTAG_JPEGIFBYTECOUNT tag is optional,
				// since it's obsolete and
				// is often missing
				...
				if (size.length > 1)
					throw new IOException(MessageLocalization.getComposedMessage(
							"compression.jpeg.is.only.supported.with.a.single.strip.this.image.has.1.strips", size.length));
				...
				if (bitsPerSample == 1 && samplePerPixel == 1) {
					img = Image.getInstance(w, h, false, Image.CCITTG4, photometric == TIFFConstants.PHOTOMETRIC_MINISBLACK ? Image.CCITT_BLACKIS1 : 0,
							g4.close());
				} else {
					zip.close();
