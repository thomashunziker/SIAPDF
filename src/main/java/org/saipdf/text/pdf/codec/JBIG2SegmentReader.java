...
package org.saipdf.text.pdf.codec;
...
import org.saipdf.text.pdf.RandomAccessFileOrArray;
 ...
 * what the width and height of each page is, and global segments if there are
 * any. Or: the minimum required to be able to take a normal sequential or
 * random-access organized file, and be able to embed JBIG2 pages as images in a
 * PDF.
 * 
...
public class JBIG2SegmentReader {
	...
	public static class JBIG2Page {
		...
		/**
		 * return as a single byte array the header-data for each segment in
		 * segment number order, EMBEDDED organization, but i am putting the
		 * needed segments in SEQUENTIAL organization. if for_embedding, skip
		 * the segment types that are known to be not for acrobat.
		 * 
		 * @param for_embedding
		 * @return a byte array
		 * @throws IOException
	...
	JBIG2Segment readHeader() throws IOException {
		...
		if (count_of_referred_to_segments == 7) {
			...
			throw new IllegalStateException(MessageLocalization.getComposedMessage(
					"count.of.referred.to.segments.had.bad.value.in.header.for.segment.1.starting.at.2", String.valueOf(segment_number), String.valueOf(ptr)));
		}
		...
		for (int i = 1; i <= count_of_referred_to_segments; i++) {
			if (segment_number <= 256) {
				...
				referred_to_segment_numbers[i] = (int) ra.readUnsignedInt(); // TODO
																				// wtf
																				// ack
			}
		}
		...
		if (segment_page_association < 0) {
			throw new IllegalStateException(MessageLocalization.getComposedMessage("page.1.invalid.for.segment.2.starting.at.3",
		...
		// TODO the 0xffffffff value that might be here, and how to understand
		// those afflicted segments
