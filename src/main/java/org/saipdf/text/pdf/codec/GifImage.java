...
/**
 * Reads gif images of all types. All the images in a gif are read in the
 * constructors and can be retrieved with other methods.
 * 
...
public class GifImage {
	...
	protected ArrayList frames = new ArrayList(); // frames read from current
													// file
	...
	/**
	 * Gets the logical screen. The images may be smaller and placed in some
	 * position in this screen to playback some animation. No image will be be
	 * bigger that this.
	 * 
	...
	protected void readImage() throws IOException {
		...
		if (transparency && transIndex >= m_curr_table.length / 3)
			transparency = false;
		if (transparency && m_bpc == 1) { // Acrobat 5.05 doesn't like this
											// combination
			byte tp[] = new byte[12];
