...
import org.saipdf.text.Annotation;
import org.saipdf.text.DocumentException;
import org.saipdf.text.Element;
...
import org.saipdf.text.exceptions.IllegalPdfSyntaxException;
import org.saipdf.text.pdf.internal.PdfAnnotationsImp;
import org.saipdf.text.pdf.internal.PdfXConformanceImp;


/**
 * <CODE>PdfContentByte</CODE> is an object containing the user positioned text
...
public class PdfContentByte {
	 ...
	 * <CODE>PdfContentByte</CODE>-object.
	 * 
	...
	/**
	 * Returns the PDF representation of this <CODE>PdfContentByte</CODE>
	 * -object.
	 * 
	 * @param writer
	...
	/**
	 * Adds the content of another <CODE>PdfContent</CODE>-object to this
	 * object.
	 * 
	 * @param other
	 *            another <CODE>PdfByteContent</CODE>-object
	 */
	 ...
	 * <P>
	 * <VAR>Flatness</VAR> sets the maximum permitted distance in device pixels
	 * between the mathematically correct path and an approximation constructed
	 * from straight line segments.<BR>
	 * 
	 * @param flatness
	 *            a value
	 */
	 ...
	 * <P>
	 * The <VAR>line cap style</VAR> specifies the shape to be used at the end
	 * of open subpaths when they are stroked.<BR>
	 * Allowed values are LINE_CAP_BUTT, LINE_CAP_ROUND and
	 * LINE_CAP_PROJECTING_SQUARE.<BR>
	 * 
	 * @param style
	 *            a value
	 */
	 ...
	 * stroke paths. It is specified by an <I>array</I> and a <I>phase</I>. The
	 ...
	 *            the value of the phase
	 * @param unitsOn
	 *            the number of units that must be 'on' (equals the number of
	 *            units that must be 'off').
	 */
	 ...
	 * <P>
	 * The <VAR>line join style</VAR> specifies the shape to be used at the
	 * corners of paths that are stroked.<BR>
	 * Allowed values are LINE_JOIN_MITER (Miter joins), LINE_JOIN_ROUND (Round
	 ...
	 * <P>
	 * The line width specifies the thickness of the line used to stroke a path
	 * and is measured in user space units.<BR>
	 * 
	 * @param w
	 *            a width
	 */
	 ...
	 * far beyond the thickness of the line stroking path. The miter limit
	 * imposes a maximum on the ratio of the miter length to the line witdh.
	 * When the limit is exceeded, the join is converted from a miter to a
	 * bevel.<BR>
	 * 
	 * @param miterLimit
	 *            a miter limit
	...
	/**
	 * Changes the currentgray tint for filling paths (device dependent
	 * colors!).
	 * <P>
	 * Sets the color space to <B>DeviceGray</B> (or the <B>DefaultGray</B>
	 * color space), and sets the gray tint to use for filling paths.
	 * </P>
	 * 
	 * @param gray
	...
	/**
	 * Changes the currentgray tint for stroking paths (device dependent
	 * colors!).
	 * <P>
	 * Sets the color space to <B>DeviceGray</B> (or the <B>DefaultGray</B>
	 * color space), and sets the gray tint to use for stroking paths.
	 * </P>
	 * 
	 * @param gray
	 *            a value between 0 (black) and 1 (white)
	 ...
	 * <P>
	 * Sets the color space to <B>DeviceRGB</B> (or the <B>DefaultRGB</B> color
	 * space), and sets the color to use for filling paths.
	 * </P>
	 * <P>
	 * Following the PDF manual, each operand must be a number between 0
	 ...
	 * <P>
	 * Sets the color space to <B>DeviceRGB</B> (or the <B>DefaultRGB</B> color
	 * space), and sets the color to use for stroking paths.
	 * </P>
	 * <P>
	 * Following the PDF manual, each operand must be a number between 0
	 * (miniumum intensity) and 1 (maximum intensity).
	 * 
	 * @param red
	 *            the intensity of red. A value between 0 and 1
	 * @param green
	 ...
	 * Changes the current color for filling paths (device dependent colors!).
	 * <P>
	 * Sets the color space to <B>DeviceCMYK</B> (or the <B>DefaultCMYK</B>
	 * color space), and sets the color to use for filling paths.
	 * </P>
	 * <P>
	 * Following the PDF manual, each operand must be a number between 0 (no
	 * ink) and 1 (maximum ink).
	 * </P>
	 * 
	 * @param cyan
	 *            the intensity of cyan. A value between 0 and 1
	 ...
	 * <P>
	 * Sets the color space to <B>DeviceCMYK</B> (or the <B>DefaultCMYK</B>
	 * color space), and sets the color to use for stroking paths.
	 * </P>
	 * <P>
	 * Following the PDF manual, each operand must be a number between 0
	 * (miniumum intensity) and 1 (maximum intensity).
	...
	/**
	 * Move the current point <I>(x, y)</I>, omitting any connecting line
	 * segment.
	 * 
	 * @param x
	 *            new x-coordinate
	...
	/**
	 * Appends a straight line segment from the current point <I>(x, y)</I>. The
	 * new current point is <I>(x, y)</I>.
	 * 
	 * @param x
	 *            new x-coordinate
	 * @param y
	 ...
	 * {@link org.saipdf.text.Rectangle#isUseVariableBorders()
	 * Rectangle.isUseVariableBorders} = true.
	 * 
	 * @param rect
	...
	public void rectangle(Rectangle rectangle) {
		...
		if (rectangle.isUseVariableBorders()) {
			...
			// if the border isn't a rectangle, the different sides are added
			// apart
			else {
	...
	/**
	 * Closes the path, fills it using the non-zero winding number rule to
	 * determine the region to fill and strokes it.
	 */
	...
	/**
	 * Closes the path, fills it using the even-odd rule to determine the region
	 * to fill and strokes it.
	 */
	 ...
	 *             if the <CODE>Image</CODE> does not have absolute positioning
	 */
	 ...
	 *            the <CODE>Image</CODE> object
	 * @param inlineImage
	 *            <CODE>true</CODE> to place this image inline,
	 ...
	 * <CODE>Image</CODE> is done with the transformation matrix. To position an
	 * <CODE>image</CODE> at (x,y) use addImage(image, image_width, 0, 0,
	 * image_height, x, y).
	 * 
	 * @param image
	 *            the <CODE>Image</CODE> object
	 ...
	 * <CODE>Image</CODE> is done with the transformation matrix. To position an
	 * <CODE>image</CODE> at (x,y) use addImage(image, image_width, 0, 0,
	 * image_height, x, y). The image can be placed inline.
	 * 
	 * @param image
	 *            the <CODE>Image</CODE> object
	 * @param a
				...
				if (inlineImage) {
					...
					for (Iterator it = pimage.getKeys().iterator(); it.hasNext();) {
						...
						if (key.equals(PdfName.COLORSPACE) && value.isArray()) {
							...
							if (ar.size() == 4 && PdfName.INDEXED.equals(ar.getAsName(0)) && ar.getPdfObject(1).isName() && ar.getPdfObject(2).isNumber()
	...
	/**
	 * Makes this <CODE>PdfContentByte</CODE> empty. Calls
	 ...
	 *            will call <code>sanityCheck()</code> if true.
	 * @since 2.1.6
	 */
	 ...
	 * <P>
	 * The leading parameter is measured in text space units. It specifies the
	...
	/**
	 * Moves to the next line and shows text string, using the given values of
	 * the character and word spacing parameters.
	 * 
	 * @param wordSpacing
	 *            a parameter
	 * @param charSpacing
	...
	public void setTextMatrix(float a, float b, float c, float d, float x, float y) {
		state.xTLM = x;
		state.yTLM = y;
		content.append(a).append(' ').append(b).append_i(' ').append(c).append_i(' ').append(d).append_i(' ').append(x).append_i(' ').append(y).append(" Tm")
				.append_i(separator);
	}
	 ...
	 * 
	 * @param b
	 *            the <CODE>byte</CODE> array to escape
	...
	/**
	 * Computes the width of the given string taking in account the current
	 * values of "Character spacing", "Word Spacing" and "Horizontal Scaling".
	 * The additional spacing is not computed for the last character of the
	 * string.
	 * 
	 * @param text
	 ...
	 * <P>
	 * (x1, y1) and (x2, y2) are the corners of the enclosing rectangle. Angles,
	 * measured in degrees, start with 0 to the right (the positive X axis) and
	 * increase counter-clockwise. The arc extends from startAng to
	 * startAng+extent. I.e. startAng=0 and extent=180 yields an openside-down
	 * semi-circle.
	 * <P>
	 * The resulting coordinates are of the form float[]{x1,y1,x2,y2,x3,y3,
	 * x4,y4} such that the curve goes from (x1, y1) to (x4, y4) with (x2, y2)
	 * and (x3, y3) as their respective Bezier control points.
	 * <P>
	 * Note: this code was taken from ReportLab (www.reportlab.org), an
	 * excellent PDF generator for Python (BSD license:
	...
	public static ArrayList bezierArc(float x1, float y1, float x2, float y2, float startAng, float extent) {
		...
		for (int i = 0; i < Nfrag; ++i) {
			...
			if (fragAngle > 0f) {
				pointList.add(new float[] { x_cen + rx * cos0, y_cen - ry * sin0, x_cen + rx * (cos0 - kappa * sin0), y_cen - ry * (sin0 + kappa * cos0),
						x_cen + rx * (cos1 + kappa * sin1), y_cen - ry * (sin1 - kappa * cos1), x_cen + rx * cos1, y_cen - ry * sin1 });
			} else {
				pointList.add(new float[] { x_cen + rx * cos0, y_cen - ry * sin0, x_cen + rx * (cos0 + kappa * sin0), y_cen - ry * (sin0 - kappa * cos0),
						x_cen + rx * (cos1 - kappa * sin1), y_cen - ry * (sin1 + kappa * cos1), x_cen + rx * cos1, y_cen - ry * sin1 });
			}
	 ...
	 * starting at startAng degrees and covering extent degrees. Angles start
	 * with 0 to the right (+x) and increase counter-clockwise.
	 * 
	 * @param x1
	 *            a corner of the enclosing rectangle
	 * @param y1
	 ...
	 * @param xstep
	 *            the desired horizontal spacing between pattern cells. May be
	 *            either positive or negative, but not zero.
	 * @param ystep
	 *            the desired vertical spacing between pattern cells. May be
	 *            either positive or negative, but not zero.
	 * @return the <CODE>PdfPatternPainter</CODE> where the pattern will be
	 *         created
	 */
	...
	/**
	 * Create a new colored tiling pattern. Variables xstep and ystep are set to
	 * the same values of width and height.
	 * 
	 * @param width
	 *            the width of the pattern
	 * @param height
	 ...
	 *            either positive or negative, but not zero.
	 * @param color
	 *            the default color. Can be <CODE>null</CODE>
	 * @return the <CODE>PdfPatternPainter</CODE> where the pattern will be
	 *         created
	 */
	...
	/**
	 * Create a new uncolored tiling pattern. Variables xstep and ystep are set
	 * to the same values of width and height.
	 * 
	 * @param width
	 *            the width of the pattern
	 * @param height
	 ...
	 * <P>
	 * Creates a new template that is nothing more than a form XObject. This
	 * template can be included in this <CODE>PdfContentByte</CODE> or in
	 * another template. Templates are only written to the output when the
	 * document is closed permitting things like showing text in the first page
	 ...
	 * </P>
	 * <P>
	 * This method is described in the 'Portable Document Format Reference
	 * Manual version 1.3' section 8.5.2.1 (page 331).
	 * </P>
	 * <P>
	 * Following the PDF manual, each operand must be a number between 0 (no
	 * ink) and 1 (maximum ink). This method however accepts only integers
	 ...
	 * </P>
	 * Following the PDF manual, each operand must be a number between 0
	 * (minimum intensity) and 1 (maximum intensity). This method however
	 * accepts only integers between 0x00 and 0xFF.
	 * 
	 * @param cyan
	...
	/**
	 * Sets the fill color to a pattern. The pattern can be colored or
	 * uncolored.
	 * 
	 * @param p
	 *            the pattern
	...
	/**
	 * Sets the stroke color to a pattern. The pattern can be colored or
	 * uncolored.
	 * 
	 * @param p
	 *            the pattern
	...
	/**
	 * Implements a link to other part of the document. The jump will be made to
	 * a local destination with the same name, that must exist.
	 * 
	 * @param name
	 ...
	 * @param destination
	 *            the <CODE>PdfDestination</CODE> with the jump coordinates
	...
	/**
	 * Gets a duplicate of this <CODE>PdfContentByte</CODE>. All the members are
	 * copied by reference but the buffer stays different.
	 * 
	...
	/**
	 * Gets a <CODE>Graphics2D</CODE> to write on. The graphics are translated
	 * to PDF commands as shapes. No PDF fonts will appear.
	 * 
	 * @param width
	...
	/**
	 * Gets a <CODE>Graphics2D</CODE> to print on. The graphics are translated
	 * to PDF commands as shapes. No PDF fonts will appear.
	 * 
	 * @param width
	 *            the width of the panel
	 ...
	 *            the height of the panel
	 * @param fontMapper
	 *            the mapping from awt fonts to <CODE>BaseFont</CODE>
	 * @return a <CODE>Graphics2D</CODE>
	 */
	 ...
	 * <CODE>layer</CODE>. Blocks can be nested. Each block must be terminated
	 * by an {@link #endLayer()}.
	 * <p>
	 * Note that nested layers with {@link PdfLayer#addChild(PdfLayer)} only
	 * require a single call to this method and a single call to
	 * {@link #endLayer()}; all the nesting control is built in.
	 * 
	 * @param layer
	 *            the layer
	 */
	...
	/**
	 * Ends a layer controlled graphic block. It will end the most recent open
	 * block.
	 */
	...
	/**
	 * Begins a marked content sequence. This sequence will be tagged with the
	 * structure <CODE>struc</CODE>. The same structure can be used several
	 * times to connect text that belongs to the same logical segment but is in
	 * a different location, like the same paragraph crossing to another page,
	 * for example.
	 * 
	 * @param struc
	...
	/**
	 * Begins a marked content sequence. If property is <CODE>null</CODE> the
	 * mark will be of the type <CODE>BMC</CODE> otherwise it will be
	 * <CODE>BDC</CODE>.
	 * 
	 * @param tag
	 *            the tag
	 ...
	 *            the property
	 * @param inline
	 *            <CODE>true</CODE> to include the property in the content or
	 *            <CODE>false</CODE> to include the property in the resource
	 *            dictionary with the possibility of reusing
	 */
	 ...
	 * <CODE>beginMarkedContentSequence(tag, null, false)</CODE>.
	 * 
	 * @param tag
	 *            the tag
	 */
	...
	/**
	 * Checks for any dangling state: Mismatched save/restore state, begin/end
	 * text, begin/end layer, or begin/end marked content sequence. If found,
	 * this function will throw. This function is called automatically during a
	 * reset() (from Document.newPage() for example), and before writing itself
	 * out in toPdf(). One possible cause: not calling myPdfGraphics2D.dispose()
	 * will leave dangling saveState() calls.
	 * 
	 * @since 2.1.6
