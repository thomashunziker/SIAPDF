...
/**
 * <CODE>PdfLine</CODE> defines an array with <CODE>PdfChunk</CODE>-objects that
 * fit into 1 line.
 */
...
public class PdfLine {
	 ...
	 * 
	 * @param left
	 *            the limit of the line at the left
	 * @param right
	 *            the limit of the line at the right
	 * @param alignment
	 *            the alignment of the line
	 * @param height
	 *            the height of the line
	 ...
	 *            bigger than 0 if the line isn't completely filled
	 * @param alignment
	 *            the alignment of the line
	 * @param newlineSplit
	 *            was the line splitted (or does the paragraph end with this
	 *            line)
	 * @param line
	 *            an array of PdfChunk objects
	 * @param isRTL
	 ...
	 * 
	 * @param chunk
	 *            the <CODE>PdfChunk</CODE> to add
	 * @return <CODE>null</CODE> if the chunk could be added completely; if not
	 *         a <CODE>PdfChunk</CODE> containing the part of the chunk that
	 *         could not be added is returned
	 */
	...
	PdfChunk add(PdfChunk chunk) {
		...
		// if the length == 0 and there were no other chunks added to the line
		// yet,
		// we risk to end up in an endless loop trying endlessly to add the same
		// chunk
		else if (line.size() < 1) {
			chunk = overflow;
			...
			// if the chunk couldn't even be truncated, we add everything, so be
			// it
			else {
				if (overflow != null)
					addToLine(overflow);
	...
	/**
	 * Returns the left indentation of the line taking the alignment of the line
	 * into account.
	 * 
	 * @return a value
	 */
	 ...
	 * 
	 * @return <CODE>true</CODE> if the alignment equals
	 *         <VAR>ALIGN_JUSTIFIED</VAR> and there is some width left.
	 */
	...
	/**
	 * Returns the width that is left, after a maximum of characters is added to
	 * the line.
	 * 
	 * @return a value
	 */
	 ...
	 * 
	 * @return a <CODE>PdfChunk</CODE> if the line has a listsymbol;
	...
	/*
	 * Gets the maximum size of all the fonts used in this line including
	 * images.
	 * 
	 * @return maximum size of all the fonts used in this line float
	 * getMaxSizeSimple() { float maxSize = 0; PdfChunk chunk; for (int k = 0; k
	 * < line.size(); ++k) { chunk = (PdfChunk)line.get(k); if
	 * (!chunk.isImage()) { maxSize = Math.max(chunk.font().size(), maxSize); }
	 * else { maxSize = Math.max(chunk.getImage().getScaledHeight() +
	 * chunk.getImageOffsetY() , maxSize); } } return maxSize; }
	 */

	/**
	 * Gets the difference between the "normal" leading and the maximum size
	 * (for instance when there are images in the chunk).
	 * 
	...
	/**
	 * Gets the maximum size of the ascender for all the fonts used in this
	 * line.
	 * 
	...
	/**
	 * Gets the biggest descender for all the fonts used in this line. Note that
