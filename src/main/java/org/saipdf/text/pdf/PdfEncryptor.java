...
/**
 * This class takes any PDF and returns exactly the same but encrypted. All the
 * content, links, outlines, etc, are kept. It is also possible to change the
 * info dictionary.
 */
public final class PdfEncryptor {
	...
	/**
	 * Entry point to encrypt a PDF document. The encryption parameters are the
	 * same as in <code>PdfWriter</code>. The userPassword and the ownerPassword
	 * can be null or have zero length. In this case the ownerPassword is
	 * replaced by a random string. The open permissions for the document can be
	 ...
	 *            for 40 bit key length
	 * @param newInfo
	 *            an optional <CODE>String</CODE> map to add or change the info
	 *            dictionary. Entries with <CODE>null</CODE> values delete the
	 *            key in the original info dictionary
	 * @throws DocumentException
	 *             on error
	 * @throws IOException
	 *             on error
	 */
	public static void encrypt(PdfReader reader, OutputStream os, byte userPassword[], byte ownerPassword[], int permissions, boolean strength128Bits,
			HashMap newInfo) throws DocumentException, IOException {
		PdfStamper stamper = new PdfStamper(reader, os);
		stamper.setEncryption(userPassword, ownerPassword, permissions, strength128Bits);
	 ...
	 *            the owner password. Can be null or empty
	 * @param permissions
	 *            the user permissions values delete the key in the original
	 *            info dictionary
	 * @throws DocumentException
	 *             on error
	 * @throws IOException
