...
public class PdfCopy extends PdfWriter {
	...
	/**
	 * Translate a PRIndirectReference to a PdfIndirectReference In addition,
	 * translates the object numbers, and copies the referenced object to the
	 * output file. NB: PRIndirectReferences (and PRIndirectObjects) really need
	 * to know what file they came from, because each file has its own
	 * namespace. The translation we do from their namespace to ours is *at
	 * best* heuristic, and guaranteed to fail under some circumstances.
	 */
	...
	/**
	 * Translate a PRArray to a PdfArray. Also translate all of the objects
	 * contained in it
	 */
	...
	/*
	 * the getCatalog method is part of PdfWriter. we wrap this so that we can
	 * extend it
	 */
	...
	/**
	 * Create a page stamp. New content and annotations, including new fields,
	 * are allowed. The fields added cannot have parents in another pages. This
	 * method modifies the PdfReader instance.
	 * <p>
					...
					if (annot.isAnnotation()) {
						...
						if (!annot.isUsed()) {
							...
							if (rect != null && (rect.left() != 0 || rect.right() != 0 || rect.top() != 0 || rect.bottom() != 0)) {
								...
								switch (rotation) {
								case 90:
									annot.put(PdfName.RECT, new PdfRectangle(pageSize.getTop() - rect.bottom(), rect.left(), pageSize.getTop() - rect.top(),
											rect.right()));
									break;
								case 180:
									annot.put(
											PdfName.RECT,
											new PdfRectangle(pageSize.getRight() - rect.left(), pageSize.getTop() - rect.bottom(), pageSize.getRight()
													- rect.right(), pageSize.getTop() - rect.top()));
									break;
								case 270:
									annot.put(PdfName.RECT, new PdfRectangle(rect.bottom(), pageSize.getRight() - rect.left(), rect.top(), pageSize.getRight()
											- rect.right()));
									break;
								}
							}
	...
	public static class StampContent extends PdfContentByte {
		...
		/**
		 * Gets a duplicate of this <CODE>PdfContentByte</CODE>. All the members
