...
public class TernaryTree implements Cloneable, Serializable {
	 ...
	 * since 0xFFFF is guaranteed not to be an Unicode character.
	 * </p>
	 */
	protected char[] sc;
	...
	private char insert(char p, char[] key, int start, char val) {
		...
		if (p == 0) {
			...
			if (len > 0) {
				...
				lo[p] = (char) kv.alloc(len + 1); // use 'lo' to hold pointer to
													// key
	...
	/*
	 * public static void main(String[] args) throws Exception { TernaryTree tt
	 * = new TernaryTree(); tt.insert("Carlos", 'C'); tt.insert("Car", 'r');
	 * tt.insert("palos", 'l'); tt.insert("pa", 'p'); tt.trimToSize();
	 * System.out.println((char)tt.find("Car"));
	 * System.out.println((char)tt.find("Carlos"));
	 * System.out.println((char)tt.find("alto")); tt.printStats(); }
	 */
