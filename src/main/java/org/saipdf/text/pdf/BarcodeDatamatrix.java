...
import org.saipdf.text.BadElementException;
import org.saipdf.text.Image;
import org.saipdf.text.pdf.codec.CCITTG4Encoder;
...
public class BarcodeDatamatrix {
	...
	private final static DmParams[] dmSizes = { new DmParams(10, 10, 10, 10, 3, 3, 5), new DmParams(12, 12, 12, 12, 5, 5, 7),
			new DmParams(8, 18, 8, 18, 5, 5, 7), new DmParams(14, 14, 14, 14, 8, 8, 10), new DmParams(8, 32, 8, 16, 10, 10, 11),
			new DmParams(16, 16, 16, 16, 12, 12, 12), new DmParams(12, 26, 12, 26, 16, 16, 14), new DmParams(18, 18, 18, 18, 18, 18, 14),
			new DmParams(20, 20, 20, 20, 22, 22, 18), new DmParams(12, 36, 12, 18, 22, 22, 18), new DmParams(22, 22, 22, 22, 30, 30, 20),
			new DmParams(16, 36, 16, 18, 32, 32, 24), new DmParams(24, 24, 24, 24, 36, 36, 24), new DmParams(26, 26, 26, 26, 44, 44, 28),
			new DmParams(16, 48, 16, 24, 49, 49, 28), new DmParams(32, 32, 16, 16, 62, 62, 36), new DmParams(36, 36, 18, 18, 86, 86, 42),
			new DmParams(40, 40, 20, 20, 114, 114, 48), new DmParams(44, 44, 22, 22, 144, 144, 56), new DmParams(48, 48, 24, 24, 174, 174, 68),
			new DmParams(52, 52, 26, 26, 204, 102, 42), new DmParams(64, 64, 16, 16, 280, 140, 56), new DmParams(72, 72, 18, 18, 368, 92, 36),
			new DmParams(80, 80, 20, 20, 456, 114, 48), new DmParams(88, 88, 22, 22, 576, 144, 56), new DmParams(96, 96, 24, 24, 696, 174, 68),
			new DmParams(104, 104, 26, 26, 816, 136, 56), new DmParams(120, 120, 20, 20, 1050, 175, 68), new DmParams(132, 132, 22, 22, 1304, 163, 62),
	...
	/**
	 * Creates a barcode. The <CODE>String</CODE> is interpreted with the
	 * ISO-8859-1 encoding
	 * 
	 * @param text
	 *            the text
	 ...
	 *         <CODE>DM_ERROR_TEXT_TOO_BIG</CODE> - the text is too big for the
	 *         symbology capabilities.<br>
	 *         <CODE>DM_ERROR_INVALID_SQUARE</CODE> - the dimensions given for
	 *         the symbol are illegal.<br>
	 *         <CODE>DM_ERROR_EXTENSION</CODE> - an error was while parsing an
	 *         extension.
	...
	/**
	 * Gets an <CODE>Image</CODE> with the barcode. A successful call to the
	 * method <CODE>generate()</CODE> before calling this method is required.
	 * 
	...
	/**
	 * Gets the generated image. The image is represented as a stream of bytes,
	 * each byte representing 8 pixels, 0 for white and 1 for black, with the
	 * high-order bit of each byte first. Each row is aligned at byte
	 * boundaries. The dimensions of the image are defined by height and width
	 * plus 2 * ws.
	 * 
	...
	/**
	 * Gets the height of the barcode. Will contain the real height used after a
	 * successful call to <CODE>generate()</CODE>. This height doesn't include
	 * the whitespace border, if any.
	 * 
	...
	/**
	 * Sets the height of the barcode. If the height is zero it will be
	 * calculated. This height doesn't include the whitespace border, if any.
	 * <p>
	...
	/**
	 * Gets the width of the barcode. Will contain the real width used after a
	 * successful call to <CODE>generate()</CODE>. This width doesn't include
	 * the whitespace border, if any.
	 * 
	...
	/**
	 * Sets the width of the barcode. If the width is zero it will be
	 * calculated. This width doesn't include the whitespace border, if any.
	 * <p>
	 * The allowed dimensions are (height, width):
	 * <p>
	 * 10, 10<br>
	 ...
	 * <CODE>DM_RAW</CODE> - no encodation. The bytes provided are already
	 * encoded and will be added directly to the barcode, using padding if
	 * needed. It assumes that the encodation state is left at ASCII after the
	 * last byte.<br>
	 * <p>
	 * One of:<br>
	 * <CODE>DM_EXTENSION</CODE> - allows extensions to be embedded at the start
	 * of the text:
	 * <p>
	 ...
	 * m6 - macro 6<br>
	 * f - FNC1<br>
	 * saabbccccc - Structured Append, aa symbol position (1-16), bb total
	 * number of symbols (2-16), ccccc file identification (0-64515)<br>
	 * p - Reader programming<br>
	 * . - extension terminator
	 * <p>
	 * Example for a structured append, symbol 2 of 6, with FNC1 and ECI 000005.
	 * The actual text is "Hello".
	 * <p>
	 * s020600075fe000005.Hello
	 * <p>
	 * One of:<br>
	 * <CODE>DM_TEST</CODE> - doesn't generate the image but returns all the
	 * other information.
	 * 
	 * @param options
	 *            the barcode options
	...
	static class Placement {
		...
		/*
		 * "ECC200" fills an nrow x ncol array with appropriate values for
		 * ECC200
		 */
		private void ecc200() {
			int row, col, chr;
			...
			do {
				...
				col += 3;
				/*
				 * & then sweep downward diagonally, inserting successive
				 * characters,...
				 */
	...
	static class ReedSolomon {
		...
		private static final int log[] = { 0, 255, 1, 240, 2, 225, 241, 53, 3, 38, 226, 133, 242, 43, 54, 210, 4, 195, 39, 114, 227, 106, 134, 28, 243, 140,
				44, 23, 55, 118, 211, 234, 5, 219, 196, 96, 40, 222, 115, 103, 228, 78, 107, 125, 135, 8, 29, 162, 244, 186, 141, 180, 45, 99, 24, 49, 56, 13,
				119, 153, 212, 199, 235, 91, 6, 76, 220, 217, 197, 11, 97, 184, 41, 36, 223, 253, 116, 138, 104, 193, 229, 86, 79, 171, 108, 165, 126, 145,
				136, 34, 9, 74, 30, 32, 163, 84, 245, 173, 187, 204, 142, 81, 181, 190, 46, 88, 100, 159, 25, 231, 50, 207, 57, 147, 14, 67, 120, 128, 154,
				248, 213, 167, 200, 63, 236, 110, 92, 176, 7, 161, 77, 124, 221, 102, 218, 95, 198, 90, 12, 152, 98, 48, 185, 179, 42, 209, 37, 132, 224, 52,
				254, 239, 117, 233, 139, 22, 105, 27, 194, 113, 230, 206, 87, 158, 80, 189, 172, 203, 109, 175, 166, 62, 127, 247, 146, 66, 137, 192, 35, 252,
				10, 183, 75, 216, 31, 83, 33, 73, 164, 144, 85, 170, 246, 65, 174, 61, 188, 202, 205, 157, 143, 169, 82, 72, 182, 215, 191, 251, 47, 178, 89,
				151, 101, 94, 160, 123, 26, 112, 232, 21, 51, 238, 208, 131, 58, 69, 148, 18, 15, 16, 68, 17, 121, 149, 129, 19, 155, 59, 249, 70, 214, 250,
				168, 71, 201, 156, 64, 60, 237, 130, 111, 20, 93, 122, 177, 150 };

		private static final int alog[] = { 1, 2, 4, 8, 16, 32, 64, 128, 45, 90, 180, 69, 138, 57, 114, 228, 229, 231, 227, 235, 251, 219, 155, 27, 54, 108,
				216, 157, 23, 46, 92, 184, 93, 186, 89, 178, 73, 146, 9, 18, 36, 72, 144, 13, 26, 52, 104, 208, 141, 55, 110, 220, 149, 7, 14, 28, 56, 112,
				224, 237, 247, 195, 171, 123, 246, 193, 175, 115, 230, 225, 239, 243, 203, 187, 91, 182, 65, 130, 41, 82, 164, 101, 202, 185, 95, 190, 81, 162,
				105, 210, 137, 63, 126, 252, 213, 135, 35, 70, 140, 53, 106, 212, 133, 39, 78, 156, 21, 42, 84, 168, 125, 250, 217, 159, 19, 38, 76, 152, 29,
				58, 116, 232, 253, 215, 131, 43, 86, 172, 117, 234, 249, 223, 147, 11, 22, 44, 88, 176, 77, 154, 25, 50, 100, 200, 189, 87, 174, 113, 226, 233,
				255, 211, 139, 59, 118, 236, 245, 199, 163, 107, 214, 129, 47, 94, 188, 85, 170, 121, 242, 201, 191, 83, 166, 97, 194, 169, 127, 254, 209, 143,
				51, 102, 204, 181, 71, 142, 49, 98, 196, 165, 103, 206, 177, 79, 158, 17, 34, 68, 136, 61, 122, 244, 197, 167, 99, 198, 161, 111, 222, 145, 15,
				30, 60, 120, 240, 205, 183, 67, 134, 33, 66, 132, 37, 74, 148, 5, 10, 20, 40, 80, 160, 109, 218, 153, 31, 62, 124, 248, 221, 151, 3, 6, 12, 24,
				48, 96, 192, 173, 119, 238, 241, 207, 179, 75, 150, 1 };
		...
		private static final int poly28[] = { 211, 231, 43, 97, 71, 96, 103, 174, 37, 151, 170, 53, 75, 34, 249, 121, 17, 138, 110, 213, 141, 136, 120, 151,
				233, 168, 93, 255 };

		private static final int poly36[] = { 245, 127, 242, 218, 130, 250, 162, 181, 102, 120, 84, 179, 220, 251, 80, 182, 229, 18, 2, 4, 68, 33, 101, 137,
				95, 119, 115, 44, 175, 184, 59, 25, 225, 98, 81, 112 };

		private static final int poly42[] = { 77, 193, 137, 31, 19, 38, 22, 153, 247, 105, 122, 2, 245, 133, 242, 8, 175, 95, 100, 9, 167, 105, 214, 111, 57,
				121, 21, 1, 253, 57, 54, 101, 248, 202, 69, 50, 150, 177, 226, 5, 9, 5 };

		private static final int poly48[] = { 245, 132, 172, 223, 96, 32, 117, 22, 238, 133, 238, 231, 205, 188, 237, 87, 191, 106, 16, 147, 118, 23, 37, 90,
				170, 205, 131, 88, 120, 100, 66, 138, 186, 240, 82, 44, 176, 87, 187, 147, 160, 175, 69, 213, 92, 253, 225, 19 };

		private static final int poly56[] = { 175, 9, 223, 238, 12, 17, 220, 208, 100, 29, 175, 170, 230, 192, 215, 235, 150, 159, 36, 223, 38, 200, 132, 54,
				228, 146, 218, 234, 117, 203, 29, 232, 144, 238, 22, 150, 201, 117, 62, 207, 164, 13, 137, 245, 127, 67, 247, 28, 155, 43, 203, 107, 233, 53,
				143, 46 };

		private static final int poly62[] = { 242, 93, 169, 50, 144, 210, 39, 118, 202, 188, 201, 189, 143, 108, 196, 37, 185, 112, 134, 230, 245, 63, 197,
				190, 250, 106, 185, 221, 175, 64, 114, 71, 161, 44, 147, 6, 27, 218, 51, 63, 87, 10, 40, 130, 188, 17, 163, 31, 176, 170, 4, 107, 232, 7, 94,
				166, 224, 124, 86, 47, 11, 204 };

		private static final int poly68[] = { 220, 228, 173, 89, 251, 149, 159, 56, 89, 33, 147, 244, 154, 36, 73, 127, 213, 136, 248, 180, 234, 197, 158, 177,
				68, 122, 93, 213, 15, 160, 227, 236, 66, 139, 153, 185, 202, 167, 179, 25, 220, 232, 96, 210, 231, 136, 223, 239, 181, 241, 59, 52, 172, 25,
