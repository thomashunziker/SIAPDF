...
/**
 * A simple XML and HTML parser. This parser is, like the SAX parser, an event
 ...
 * <li>It lists attributes, where attribute values can be enclosed in single or
 * double quotes
 * <li>It recognizes the <code>&lt;[CDATA[ ... ]]&gt;</code> construct
 * <li>It recognizes the standard entities: &amp;amp;, &amp;lt;, &amp;gt;,
 * &amp;quot;, and &amp;apos;, as well as numeric entities
 * <li>It maps lines ending in <code>\r\n</code> and <code>\r</code> to
 * <code>\n</code> on input, in accordance with the XML Specification, Section
 * 2.11
 * </ul>
 * <p>
 */
public final class SimpleXMLParser {
	...
	/**
	 * Creates a Simple XML parser object. Call go(BufferedReader) immediately
	 * after creation.
	 */
	...
	/**
	 * Does the actual parsing. Perform this immediately after creating the
	 * parser object.
	 */
			...
			case ENTITY:
				if (character == ';') {
					...
					if (ce == '\0')
						...
						text.append(ce);
				} else if ((character != '#' && (character < '0' || character > '9') && (character < 'a' || character > 'z') && (character < 'A' || character > 'Z'))
						|| entity.length() >= 7) {
					state = restoreState();
	...
	/**
	 * Flushes the text that is currently in the buffer. The text can be
	 * ignored, added to the document as content or as comment,... depending on
	 * the current state.
	 */
	private void flush() {
		switch (state) {
	 ...
	 * 
	 * @param start
	 *            if true we are dealing with a tag that has just been opened;
	 *            if false we are closing a tag.
	 */
	 ...
	 * @param r
	 *            the document. The encoding is already resolved. The reader is
	 *            not closed
	 * @throws IOException
	 *             on error
	 */
	 ...
	 *            the document handler
	 * @param in
	 *            the document. The encoding is deduced from the stream. The
	 ...
	 * (method found in org.apache.xerces.impl.XMLEntityManager, originally
	 * published by the Apache Software Foundation under the Apache Software
	 * License; now being used in iText under the MPL)
	 * 
	 * @param b4
