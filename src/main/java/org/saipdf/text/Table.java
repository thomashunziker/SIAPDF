...
import org.saipdf.text.pdf.PdfPCell;
import org.saipdf.text.pdf.PdfPTable;


/**
 * A <CODE>Table</CODE> is a <CODE>Rectangle</CODE> that contains
 * <CODE>Cell</CODE>s, ordered in some kind of matrix.
 * <P>
 * Tables that span multiple pages are cut into different parts automatically.
 * If you want a table header to be repeated on every page, you may not forget
 * to mark the end of the header section by using the method
 * <CODE>endHeaders()</CODE>.
 * <P>
 * The matrix of a table is not necessarily an m x n-matrix. It can contain
 * holes or cells that are bigger than the unit. Believe me or not, but it took
 * some serious thinking to make this as user friendly as possible. I hope you
 * will find the result quite simple (I love simple solutions, especially for
 * complex problems). I didn't want it to be something as complex as the Java
 ...
 * example cell with colspan 1 and rowspan 2</TD>
...
public class Table extends Rectangle implements LargeElement {
	...
	/**
	 * This Empty Cell contains the DEFAULT layout of each Cell added with the
	 * method addCell(String content).
	 */
	...
	/**
	 * Boolean to track if a table was inserted (to avoid unnecessary
	...
	/**
	 * Constructs a <CODE>Table</CODE> with a certain number of columns and a
	 * certain number of <CODE>Row</CODE>s.
	 * 
	 * @param columns
	 *            The number of columns in the table
	 * @param rows
	...
	/**
	 * @see org.saipdf.text.Element#isNestable()
	 * @since iText 2.0.8
	 */
	 ...
	 * 
	 * @param alignment
	 *            the new alignment as a <CODE>String</CODE>
	 */
	...
	/**
	 * Enables/disables automatic insertion of empty cells before table is
	 * rendered. (default = false) As some people may want to create a table,
	 * fill only a couple of the cells and don't bother with investigating which
	 * empty ones need to be added, this default behavior may be very welcome.
	 * Disabling is recommended to increase speed. (empty cells should be added
	 ...
	 * <P>
	 * You can give up relative values of borderwidths. The sum of these values
	 * will be considered 100%. The values will be recalculated as percentages
	 * of this sum.
	 * <P>
	 * example: <BLOCKQUOTE>
	 * 
	 ...
	 * </PRE>
	 * 
	 * </BLOCKQUOTE> The widths will be: a width of 50% for the first column,
	 ...
	 * <P>
	 * When a table doesn't fit a page, it is split in two parts. If you want to
	 * avoid this, you should set the <VAR>tableFitsPage</VAR> value to true.
	 * 
	 * @param fitPage
	 ...
	 * Allows you to control when a page break occurs.
	 * <P>
	 * When a cell doesn't fit a page, it is split in two parts. If you want to
	 * avoid this, you should set the <VAR>cellsFitPage</VAR> value to true.
	 * 
	 * @param fitPage
	 *            enter true if you don't want to split cells
	 */
	 ...
	 * 
	 * Normally a newline is added before you add a Table object. This newline
	 * uses the current leading. If you want to control the space between the
	 * table and the previous element yourself, you have to set the offset of
	 * this table.
	 * 
	 * @param offset
	 ...
	 * 
	 * @param convert2pdfptable
	 *            true if you want iText to try to convert the Table to a
	 *            PdfPTable
	 */
	...
	/**
	 * Adds a <CODE>Cell</CODE> to the <CODE>Table</CODE> at a certain row and
	 * column.
	 * 
	 * @param aCell
	 *            The <CODE>Cell</CODE> to add
	 * @param row
	 *            The row where the <CODE>Cell</CODE> will be added
	 * @param column
	 *            The column where the <CODE>Cell</CODE> will be added
	 * @throws BadElementException
	 */
	 ...
	 *            The <CODE>Cell</CODE> to add
	 * @param aLocation
	 *            The location where the <CODE>Cell</CODE> will be added
	 * @throws BadElementException
	 */
	 ...
	 * Adds a <CODE>Cell</CODE> to the <CODE>Table</CODE>.
	 * <P>
	 * This is a shortcut for <CODE>addCell(Cell cell)</CODE>. The
	...
	public void insertTable(Table aTable, Point aLocation) {
		...
		if (aLocation.y > columns) {
			throw new IllegalArgumentException(MessageLocalization.getComposedMessage("inserttable.wrong.columnposition.1.of.location.max.eq.2",
	 ...
	 * @return boolean <CODE>true</CODE> if the row was deleted;
	...
	private void mergeInsertedTables() {
		...
		int[] lDummyWidths = new int[columns]; // to keep track in how many new
												// cols this one will be split
		float[][] lDummyColumnWidths = new float[columns][]; // bugfix Tony
																// Copping
		int[] lDummyHeights = new int[rows.size()]; // to keep track in how many
													// new rows this one will be
													// split
		...
		for (j = 0; j < columns; j++) {
			lNewMaxColumns = 1; // value to hold in how many columns the current
								// one will be split
			...
			for (i = 0; i < rows.size(); i++) {
				if (Table.class.isInstance(((Row) rows.get(i)).getCell(j))) {
					...
					if (tmpWidths == null) {
						...
						/*
						 * if( tpI<tmpWidths.length) {
						 * System.arraycopy(tmpWidths, tpI, tmpWidthsN, totI,
						 * tmpWidths.length-tpI); totI +=tmpWidths.length-tpI; }
						 * else if(btI<cols) {
						 * System.arraycopy(lDummyTable.widths, btI, tmpWidthsN,
						 * totI, lDummyTable.widths.length-btI); totI
						 * +=lDummyTable.widths.length-btI; }
						 */
					 ...
					 * lDummyColumnWidths[j] = lDummyTable.widths; // bugfix
					 * Tony Copping }
					 */
				}
			}
		...
		for (i = 0; i < rows.size(); i++) {
			lNewMaxRows = 1; // holds value in how many rows the current one
								// will be split
			for (j = 0; j < columns; j++) {
				if (Table.class.isInstance(((Row) rows.get(i)).getCell(j))) {
					isTable = true;
		...
		if ((lTotalColumns != columns) || (lTotalRows != rows.size()) || isTable) // NO
																					// ADJUSTMENT
		{
			// ** WIDTH
			// set correct width for new columns
			// divide width over new nr of columns
			// Take new max columns of internal table and work out widths for
			// each col
			...
			int lDummyRow = 0, lDummyColumn = 0; // to remember where we are in
			...
			for (i = 0; i < rows.size(); i++) {
				...
				lNewMaxRows = 1;
				for (j = 0; j < columns; j++) {
					if (Table.class.isInstance(((Row) rows.get(i)).getCell(j))) // copy
																				// values
																				// from
																				// embedded
																				// table
					{
						...
						// Work out where columns in table table correspond to
							...
							for (int l = 0; l < lDummyTable.getDimension().width; l++) {
								...
								if (lDummyElement != null) {
									...
									if (Cell.class.isInstance(lDummyElement)) {
										...
										// Find col to add cell in and set col
										// span
										col = colMap[l];
					...
					} else // copy others values
					{
						...
						if (Cell.class.isInstance(aElement)) {
							...
							// most likely this cell covers a larger area
							// because of the row/cols splits : define
	 ...
	 * 
	 * @param aCell
	 *            the cell that has to be checked
	 * @param aLocation
	...
	private boolean isValidLocation(Cell aCell, Point aLocation) {
		...
		if (aLocation.x < rows.size()) // if false : new location is already at
										// new, not-yet-created area so no check
		{
	...
	/**
	 * Inserts a Cell in a cell-array and reserves cells defined by
	 * row-/colspan.
	 * 
	 * @param someRows
	 *            some rows
	 * @param aCell
	 *            the cell that has to be inserted
	 * @param aPosition
	 ...
	 * 
	 * @param left
	 *            this is the position of the first border at the left
	 *            (cellpadding not included)
	 * @param totalWidth
	 *            this is the space between the first border at the left and the
	 *            last border at the right (cellpadding not included)
	...
	/**
	 * @since iText 2.0.8
	 * @see org.saipdf.text.LargeElement#flushContent()
	 */
	...
	/**
	 * @since iText 2.0.8
	 * @see org.saipdf.text.LargeElement#isComplete()
	 */
	...
	/**
	 * @since iText 2.0.8
	 * @see org.saipdf.text.LargeElement#setComplete(boolean)
	 */
